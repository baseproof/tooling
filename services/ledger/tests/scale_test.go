...
Gated by BASEPROOF_TEST_DSN — skips without Postgres.
Entry count configurable via BASEPROOF_SCALE_N (default 1,000,000).
...
Run:
	...
	BASEPROOF_TEST_DSN="postgres://baseproof:baseproof@localhost:5432/baseproof_test?sslmode=disable" \
	  go test ./tests/ -v -count=1 -run TestScale -timeout 30m

	# Start smaller:
	BASEPROOF_SCALE_N=100000 go test ./tests/ -v -count=1 -run TestScale -timeout 30m
*/
package tests
...
// getScaleN returns the entry count from BASEPROOF_SCALE_N (default 1M).
func getScaleN() int {
	if v := os.Getenv("BASEPROOF_SCALE_N"); v != "" {
...
func TestScale_SDKProcessBatch(t *testing.T) {
	...
	// BASEPROOF_SCALE_N=1000 for a quicker smoke test).
	if testing.Short() {
		t.Skip("scale benchmark skipped under -short; run without -short or with BASEPROOF_SCALE_N=1000 for a smoke test")
	}
	N := getScaleN()
...
func reportTableSizes(t *testing.T, pool *pgxpool.Pool) {
	t.Helper()
	for _, table := range []string{"entry_index", "smt_leaves", "builder_cursor", "delta_window_buffers"} {
		var size string
