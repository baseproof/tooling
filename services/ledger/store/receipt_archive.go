...
front reconstruct the SAME root + proofs from object storage. Fail-closed: the
checkpoint loop writes this archive BEFORE the horizon advances and withholds the
horizon on a write error (Step 9a), since a PG-off reader's receipt proof has no PG
fallback; archives for history that PREDATES the writer are regenerated by the
backfill (G1). A missing archive degrades only the receipt endpoint, never the main
read path.
