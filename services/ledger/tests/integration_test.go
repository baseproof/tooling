...
Tests requiring Postgres skip gracefully when BASEPROOF_TEST_DSN is unset.
...
Run with Postgres:     BASEPROOF_TEST_DSN="postgres://..." go test ./tests/ -v -count=1
*/
package tests
...
import (
	...
	"github.com/baseproof/baseproof/crypto/admission"
...
func TestAdmission_ModeB_WrongLog(t *testing.T) {
	...
	err = verifyStampForTest(params, nonce, "did:baseproof:different", 8)
	if err == nil {
...
func TestEquivocation_Detection(t *testing.T) {
	// Equivocation proof shape (per baseproof/witness): two
...
func TestAnchor_PayloadContent(t *testing.T) {
	...
	// Self-contained anchor shape: source_log_did + the full cosigned
	// head (gossip.WireCosignedTreeHeadBody: all four roots + the
	// K-of-N cosignatures), with tree_head_ref retained as a provenance
	// witness. See anchor/publisher.go.
	payload := mustJSON(map[string]any{
		"anchor_type":    anchor.CosignedAnchorType,
		"source_log_did": "did:baseproof:source",
		"head": map[string]any{
			"head": map[string]any{
				"root_hash":    hexRef,
				"smt_root":     hexRef,
				...
				"signatures": []any{map[string]any{
					"pub_key_id": hexRef, "scheme_tag": 1, "sig_bytes": "55",
				}},
			},
			"ledger_endpoint": "https://source.example",
		},
		"tree_head_ref": hexRef,
	})
...
func TestCommitment_Frequency(t *testing.T) {
	...
	pub := opbuilder.NewCommitmentPublisher("did:example:op", "did:example:op", opbuilder.CommitmentPublisherConfig{IntervalEntries: 100, IntervalTime: time.Hour}, func(*envelope.Entry) error { calls++; return nil }, slog.Default()).
		WithContentStore(storage.NewInMemoryContentStore())
...
func TestOps_WriteCreditIsolation(t *testing.T) {
	...
	tx, _ := pool.Begin(ctx)
	newBal, err := cs.DeductInTx(ctx, tx, "did:example:exchange-a", 1)
	tx.Commit(ctx)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = cs.DeductInTx(ctx, tx2, "did:example:exchange-b", 1)
	tx2.Rollback(ctx)
...
// TestOps_WriteCreditAllowsOvershoot pins the "any positive balance honors
// the next deduction, even if it overshoots" rule: 200 credits + cost=256
// deduction succeeds with newBalance=-56 (instead of rejecting). The single
// over-spend window absorbs whole batches without per-entry pre-checking.
func TestOps_WriteCreditAllowsOvershoot(t *testing.T) {
	pool := skipIfNoPostgres(t)
	ctx := context.Background()
	cs := store.NewCreditStore(pool)
	const did = "did:example:exchange-overshoot"
	if _, err := cs.BulkPurchase(ctx, did, 200); err != nil {
		t.Fatal(err)
	}
	if err := cs.Deduct(ctx, did, 256); err != nil {
		t.Fatalf("Deduct(256) from 200-balance: want nil, got %v", err)
	}
	bal, _ := cs.Balance(ctx, did)
	if bal != -56 {
		t.Fatalf("post-overshoot balance: got %d, want -56", bal)
	}
}

// TestOps_WriteCreditRejectsAfterOvershoot pins the "once <= 0, reject until
// top-up" rule. The pre-deduction balance gate refuses any further deduction
// regardless of cost.
func TestOps_WriteCreditRejectsAfterOvershoot(t *testing.T) {
	pool := skipIfNoPostgres(t)
	ctx := context.Background()
	cs := store.NewCreditStore(pool)
	const did = "did:example:exchange-reject-after"
	if _, err := cs.BulkPurchase(ctx, did, 100); err != nil {
		t.Fatal(err)
	}
	if err := cs.Deduct(ctx, did, 500); err != nil {
		t.Fatalf("first deduct from positive balance: %v", err)
	}
	bal, _ := cs.Balance(ctx, did)
	if bal != -400 {
		t.Fatalf("post-overshoot balance: got %d, want -400", bal)
	}
	// Now balance is -400 — any further deduction must be rejected.
	if err := cs.Deduct(ctx, did, 1); err != store.ErrInsufficientCredits {
		t.Fatalf("deduct while negative: want ErrInsufficientCredits, got %v", err)
	}
	if err := cs.Deduct(ctx, did, 100); err != store.ErrInsufficientCredits {
		t.Fatalf("deduct cost=100 while negative: want ErrInsufficientCredits, got %v", err)
	}
}

// TestOps_WriteCreditTopUpRecovers verifies BulkPurchase that takes the
// balance back > 0 re-enables deduction (and a single new over-spend window).
func TestOps_WriteCreditTopUpRecovers(t *testing.T) {
	pool := skipIfNoPostgres(t)
	ctx := context.Background()
	cs := store.NewCreditStore(pool)
	const did = "did:example:exchange-topup"
	if _, err := cs.BulkPurchase(ctx, did, 50); err != nil {
		t.Fatal(err)
	}
	if err := cs.Deduct(ctx, did, 100); err != nil {
		t.Fatalf("first deduct: %v", err)
	}
	if err := cs.Deduct(ctx, did, 1); err != store.ErrInsufficientCredits {
		t.Fatalf("rejected-state assertion: want ErrInsufficientCredits, got %v", err)
	}
	// Top up — BulkPurchase ADDs to the (negative) balance: -50 + 200 = 150.
	if _, err := cs.BulkPurchase(ctx, did, 200); err != nil {
		t.Fatalf("top-up: %v", err)
	}
	bal, _ := cs.Balance(ctx, did)
	if bal != 150 {
		t.Fatalf("post-topup balance: got %d, want 150", bal)
	}
	// Positive again → next deduction honored.
	if err := cs.Deduct(ctx, did, 10); err != nil {
		t.Fatalf("post-topup deduct: %v", err)
	}
}

// TestOps_WriteCreditInvalidCost pins the cost>0 precondition.
func TestOps_WriteCreditInvalidCost(t *testing.T) {
	pool := skipIfNoPostgres(t)
	ctx := context.Background()
	cs := store.NewCreditStore(pool)
	const did = "did:example:exchange-invalid-cost"
	if _, err := cs.BulkPurchase(ctx, did, 100); err != nil {
		t.Fatal(err)
	}
	if err := cs.Deduct(ctx, did, 0); err == nil {
		t.Fatal("Deduct(cost=0) accepted; want error")
	}
	if err := cs.Deduct(ctx, did, -1); err == nil {
		t.Fatal("Deduct(cost=-1) accepted; want error")
	}
}
