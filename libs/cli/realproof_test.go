...
func buildRealFixture(t *testing.T, n, k int, require bool) *realFixture {
	...
	if require {
		...
		// genesis ceremony. Endorse with every witness key (the ceremony signs
		// over the canonical subset, so accumulating endorsements does not change
		// what is signed nor the derived NetworkID). The endorsements live on the
		// document; whether they survive into the proof is the SDK's build choice
		// (today: stripped — see baseproof/baseproof#51).
