...
// ("baseproof.network/schema/pre-grant-commitment/v1").
...
// ─────────────────────────────────────────────────────────────────────
// 0x0E — history-rewrite projection (O(1) read by binding)
// ─────────────────────────────────────────────────────────────────────

// historyRewriteProjKey builds a key under the history-rewrite
// projection. Keyed by content-derived binding hash so the read
// endpoint computes it identically and serves a point lookup. Same
// shape as equivProjKey for the existing 0x0B projection — keep
// the two surfaces parallel so a reviewer adding the third
// (smt-replay) finds an obvious template.
func historyRewriteProjKey(binding [32]byte) []byte {
	k := make([]byte, 2+32)
	k[0] = prefixGossipRoot
	k[1] = subHistoryRewriteProj
	copy(k[2:], binding[:])
	return k
}

// ─────────────────────────────────────────────────────────────────────
// 0x0F — smt-replay projection (O(1) read by binding)
// ─────────────────────────────────────────────────────────────────────

// smtReplayProjKey builds a key under the smt-replay projection.
// Keyed by content-derived binding hash for the same O(1)-read
// rationale as 0x0B + 0x0E. The binding for an SMTReplayFinding is
// hash(LedgerEndpoint) + every witness PubKeyID appearing in either
// head's cosignature list (SDK gossip/kind.go).
func smtReplayProjKey(binding [32]byte) []byte {
	k := make([]byte, 2+32)
	k[0] = prefixGossipRoot
	k[1] = subSMTReplayProj
	copy(k[2:], binding[:])
	return k
}
