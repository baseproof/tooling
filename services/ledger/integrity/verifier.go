...
func (v *tileVerifier) HashAt(ctx context.Context, seq uint64) ([32]byte, error) {
	...
	// Fetch returns a level-0 HASH tile: raw concatenated 32-byte leaf
	// hashes per c2sp.org/tlog-tiles — NOT the length-prefixed entry-bundle
	// format (that lives at tile/entries/ and is read by FetchEntryBundle).
	// The leaf hash at offsetInTile is therefore a direct 32-byte slice;
	// using the entry-bundle parser here misreads a hash's leading two
	// bytes as a length prefix and spuriously reports "tile truncated",
	// FATAL-ing the integrity detector on a perfectly valid tile.
	start := offsetInTile * 32
	end := start + 32
	if end > uint64(len(body)) {
		return [32]byte{}, fmt.Errorf("integrity/verifier: hash tile %d too short for offset %d: need bytes [%d:%d], tile is %d bytes",
			tileIndex, offsetInTile, start, end, len(body))
	}
	var h [32]byte
	copy(h[:], body[start:end])
	return h, nil
}
