...
// witness daemon. Skips when WITNESS_URL is unset;
...
func TestE2E_WitnessContainerHealthz(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("GET %s/healthz: %v "+
			"(is the witness container up? `docker ps | grep baseproof_e2e_witness`)",
			witnessURL, err)
	}
...
func TestE2E_HeadSyncCollectsFromContainer(t *testing.T) {
	...
	head := types.TreeHead{
		...
		SMTRoot:  [32]byte{0xE2, 0x5A, 0x7C}, // non-zero: cosign Validate rejects all-zero SMTRoot
	}
