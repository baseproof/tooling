...
// TestTesseraAdapter_InclusionProof_AtHistoricalTreeSize covers the path the
// GET /v1/tree/inclusion/{seq}?tree_size=N parameter activates: proofs against a
// NON-HEAD (historical / cosigned-horizon) tree size, not just the live head.
//
// This is its own correctness regime: at a sub-head size the tree has a
// DIFFERENT right-edge shape (different partial tiles, different orphan-promoted
// subtrees), so a proof valid at the head is NOT the proof valid at size N. The
// existing RandomSampleVerifies test only exercises treeSize == head. An auditor
// rebuilding the witness-rotation chain (libs/witnessrotation) verifies every
// rotation proof against the cosigned horizon size, so this path is load-bearing
// in production.
//
// We verify each proof against the HISTORICAL root at size N (computed
// canonically from the first N leaf hashes via transparency-dev/merkle), across
// several N spanning the imperfect right edge.
func TestTesseraAdapter_InclusionProof_AtHistoricalTreeSize(t *testing.T) {
	app, dir, _ := newTestEmbeddedAppender(t)
	ctx := context.Background()
	...
	// historicalRoot computes the canonical RFC 6962 root over the first `size`
	// leaves — the root a proof at tree_size=size must reconstruct to.
	historicalRoot := func(size uint64) []byte {
		tr := testonly.New(rfc6962.DefaultHasher)
		for i := uint64(0); i < size; i++ {
			tr.AppendData(leafData[i][:])
		}
		return tr.Hash()
	}

	// Sizes spanning the imperfect right edge: a power of two, ±1, and arbitrary.
	for _, size := range []uint64{256, 257, 300, 383, 384, 499} {
		if size > headSize {
			continue
		}
		root := historicalRoot(size)
		// Sample positions across [0, size): low, mid, and the last leaf
		// (size-1) — the orphan-prone right edge.
		for _, seq := range []uint64{0, size / 3, size / 2, size - 2, size - 1} {
			if seq >= size {
				continue
			}
			raw, err := adapter.RawInclusionProof(seq, size)
			if err != nil {
				t.Fatalf("RawInclusionProof(seq=%d, size=%d): %v", seq, size, err)
			}
			m := raw.(map[string]any)
			hexHashes := m["hashes"].([]string)
			siblings := make([][]byte, len(hexHashes))
			for j, h := range hexHashes {
				...
				if derr != nil {
					t.Fatalf("decode sibling[%d] seq=%d size=%d: %v", j, seq, size, derr)
				}
				siblings[j] = b
			}
			leafHash := rfc6962.DefaultHasher.HashLeaf(leafData[seq][:])
			if err := proof.VerifyInclusion(
				rfc6962.DefaultHasher, seq, size, leafHash, siblings, root,
			); err != nil {
				t.Fatalf("VerifyInclusion(seq=%d size=%d [historical]): %v", seq, size, err)
			}
		}
	}
}

// TestTesseraAdapter_RawInclusionProof_OutOfRangeRejected pins the bound the
// ?tree_size= handler relies on: a position >= treeSize cannot be proven.
func TestTesseraAdapter_RawInclusionProof_OutOfRangeRejected(t *testing.T) {
	app, dir, _ := newTestEmbeddedAppender(t)
	ctx := context.Background()
	for i := 0; i < 300; i++ {
		var d [32]byte
		...
		if _, err := app.AppendLeaf(ctx, d[:]); err != nil {
			t.Fatalf("AppendLeaf: %v", err)
		}
	}
	deadline := time.Now().Add(30 * time.Second)
	for time.Now().Before(deadline) {
		if h, err := app.Head(); err == nil && h.TreeSize >= 300 {
			break
		}
		time.Sleep(50 * time.Millisecond)
	}
	...
	// position == treeSize and position > treeSize must both error.
	if _, err := adapter.RawInclusionProof(100, 100); err == nil {
		t.Error("RawInclusionProof(position == treeSize) should error")
	}
	if _, err := adapter.RawInclusionProof(150, 100); err == nil {
		t.Error("RawInclusionProof(position > treeSize) should error")
	}
}
