...
for i in $(seq 1 30); do
    health="$(docker inspect --format='{{.State.Health.Status}}' baseproof_test_rustfs 2>/dev/null || true)"
...
for i in $(seq 1 30); do
    status="$(docker inspect --format='{{.State.Status}}' baseproof_test_rustfs_init 2>/dev/null || true)"
    exitcode="$(docker inspect --format='{{.State.ExitCode}}' baseproof_test_rustfs_init 2>/dev/null || echo -1)"
...
export BASEPROOF_TEST_S3_ENDPOINT='http://127.0.0.1:9000'
export BASEPROOF_TEST_S3_BUCKET='baseproof-test-bytes'
export BASEPROOF_TEST_S3_ACCESS_KEY='rustfsadmin'
export BASEPROOF_TEST_S3_SECRET_KEY='rustfsadmin'
export BASEPROOF_TEST_S3_PATH_STYLE='true'
