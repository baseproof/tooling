...
import (
	...
	"github.com/baseproof/tooling/services/ledger/tests/chaos/smt_reconstruction"
)
...
func (h *Harness) SnapshotCounts(ctx context.Context) (Counts, error) {
	...
	// Older PG may not allow SET TRANSACTION here; best-effort, ignore error.
	_, _ = tx.Exec(ctx, "SET TRANSACTION ISOLATION LEVEL SERIALIZABLE READ ONLY")
...
// BackpressureObservation reports whether the submitter saw any 503
// responses during a submission run. Some chaos tests assert ZERO
// 503s (kill-restart shouldn't break admission); the backpressure
// test asserts AT LEAST ONE 503 (witnesses-down MUST trigger
// backpressure).
