...
baseproof v0.3.0 (Jellyfish/Patricia trie).
...
The PG-backed test still requires BASEPROOF_TEST_DSN; without it it
...
func TestSMTRootStateStore_RoundTrip(t *testing.T) {
	...
	if dsn == "" {
		t.Skip("BASEPROOF_TEST_DSN not set — skipping PG-backed test")
	}
