...
#
# Required env:
#   BASEPROOF_REAL_S3_BUCKET   the bucket name (REQUIRED)
...
# s3:ListBucket on $BASEPROOF_REAL_S3_BUCKET.
#
# Usage:
#   export BASEPROOF_REAL_S3_BUCKET=my-test-bucket
...
# ── Validate inputs ───────────────────────────────────────────────
if [ -z "${BASEPROOF_REAL_S3_BUCKET:-}" ]; then
    echo "FATAL: BASEPROOF_REAL_S3_BUCKET not set"
    echo
    echo "  export BASEPROOF_REAL_S3_BUCKET=<your-bucket-name>"
    echo "  $0"
    exit 1
fi
...
echo "   bucket: ${BASEPROOF_REAL_S3_BUCKET}"
echo "   region: ${AWS_REGION:-${AWS_DEFAULT_REGION}}"
echo

# Real-S3 mode signal: BASEPROOF_TEST_S3_REAL=1.
# requireS3 detects this and uses default credential chain + virtual-host URLs.
unset BASEPROOF_TEST_S3_ENDPOINT
unset BASEPROOF_TEST_S3_PATH_STYLE
export BASEPROOF_TEST_S3_REAL=1
export BASEPROOF_TEST_S3_BUCKET="${BASEPROOF_REAL_S3_BUCKET}"
export BASEPROOF_TEST_S3_REGION="${AWS_REGION:-${AWS_DEFAULT_REGION}}"
...
echo
echo "  aws s3 ls s3://${BASEPROOF_REAL_S3_BUCKET}/test/ --recursive || echo '(no leftovers — good)'"
