...
TEST GATES:
	...
	All tests Skip when BASEPROOF_TEST_DSN is unset. No GCS/S3 needed —
...
func startE2ELedger(t *testing.T) *e2eLedger {
	...
	if dsn == "" {
		t.Skip("BASEPROOF_TEST_DSN not set — skipping e2e shipper redirect test")
	}
	...
	// ── HTTP handlers ───────────────────────────────────────────────
	// Mode-B PoW admission needs Gates.ModeBPoW + a DifficultyResolver (see
	// api/submission.go); mirror production so Mode-B submissions aren't 403'd.
