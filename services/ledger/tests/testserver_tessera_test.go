...
KEY DEPENDENCIES:
  ...
  - github.com/baseproof/tooling/services/ledger/tessera: every wrapper
...
type tesseraSlots struct {
	...
	// checkpointer is the concrete adapter the CheckpointLoop needs (RootAtSize +
	// PublishCosignedCheckpoint) — the slimmed builder MerkleAppender exposes only
	// AppendLeaf. nil under any stub path.
	checkpointer *optessera.TesseraAdapter
