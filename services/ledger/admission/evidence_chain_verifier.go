...
DESCRIPTION:
	...
	walk to Path C entries (AuthorityPath == AuthorityScopeAuthority).
	A scope-authority entry's whole point is that its signer is in
	the scope's Authority_Set; the chain from the entry back through
	the authority snapshot is the structural proof, and admission
	MUST verify the chain SHAPE (no cycles, no broken hops, depth
	bounded) before letting the entry into the log.

	This is a STRUCTURAL integrity check on the evidence DAG, NOT a
	domain-authority decision. Whether the signer is actually
	authorised — the role/capability verdict — is the judicial-
	network SubmitGate's job (it runs the SDK authority engine);
	the ledger only checks that the chain is well-formed.
...
// is true for entry. The predicate fires on Path C
// (scope-authority) entries, whose admission requires a
// structural integrity check of the evidence DAG.
//
