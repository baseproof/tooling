...
ledger binary consumes:
	...
	BASEPROOF_SOAK_BYTESTORE_BACKEND  → LEDGER_BYTE_STORE_BACKEND
	BASEPROOF_TEST_S3_BUCKET          → LEDGER_BYTE_STORE_S3_BUCKET
	BASEPROOF_TEST_S3_ENDPOINT        → LEDGER_BYTE_STORE_S3_ENDPOINT
	BASEPROOF_TEST_S3_ACCESS_KEY      → LEDGER_BYTE_STORE_S3_ACCESS_KEY
	BASEPROOF_TEST_S3_SECRET_KEY      → LEDGER_BYTE_STORE_S3_SECRET_KEY
...
// from BASEPROOF_SOAK_BYTESTORE_BACKEND + BASEPROOF_TEST_S3_*.
...
func bytestoreEnv(t *testing.T) []string {
	t.Helper()
	backend := os.Getenv("BASEPROOF_SOAK_BYTESTORE_BACKEND")
	if backend == "" {
	...
	case "s3":
		return buildS3Env(t)
	case "gcs":
		t.Skipf("chaos harness doesn't wire GCS backend (set BASEPROOF_SOAK_BYTESTORE_BACKEND=s3)")
		return nil
	default:
		t.Skipf("chaos harness needs BASEPROOF_SOAK_BYTESTORE_BACKEND=s3 (got %q)", backend)
		return nil
	}
}
...
func buildS3Env(t *testing.T) []string {
	t.Helper()
	bucket := requireEnvVar(t, "BASEPROOF_TEST_S3_BUCKET")
	...
	pathStyle := os.Getenv("BASEPROOF_TEST_S3_PATH_STYLE")
