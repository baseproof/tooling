...
func TestP4_ReplicaFailover_CursorContinuity(t *testing.T) {
	...
	// A advances to 50.
	//
	// Untyped constant on purpose: the SequenceCursor API is
	// deliberately asymmetric — AdvanceTx takes uint64 (a real,
	// non-negative sequence) while Read returns int64 (signed, so
	// -1 can encode "no sequences processed yet"; see
	// store/sequence_cursor.go:77-82). An untyped constant converts
	// to uint64 at the AdvanceTx call AND to int64 at the
	// `got != advanceTo` comparison below — matching the API on both
	// sides without a cast.
	const advanceTo = 50
...
func TestP4_ReplicaFailover_ConcurrentAcquireOnlyOneAdvances(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		go func(i int) {
			...
			// advance the cursor, report the result, THEN park
			// holding the lock until the test signals release.
			heldMu.Lock()
	...
	// Collect exactly N results. Every goroutine sends one result
	// BEFORE it returns (losers) or parks on holdRelease (the
	// winner), so this loop completes without any goroutine having
	// to exit — the winner stays parked holding the lock, which is
	// the whole point of the contention test. We must NOT gate on
	// wg.Wait() here: the winner's deferred wg.Done() can't fire
	// until holdRelease closes, and holdRelease must stay open
	// until assertions are done — gating on the WaitGroup would
	// deadlock (wg.Wait → winner Done → holdRelease close → test
	// return → wg.Wait).
	...
	// Release the parked winner, wait for all goroutines to exit,
	// then tear down their pools/locks. Done before the final
	// assertion so cleanup runs even when the winner count is wrong.
	close(holdRelease)
	wg.Wait()
