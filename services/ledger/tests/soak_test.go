...
it — soak runs are minutes-long and cost real cloud spend. Opt-in via:
	...
	BASEPROOF_TEST_GCS_BUCKET=baseproof-soak-<your-instance>  \
...
CONFIG VIA ENV (with defaults):
	...
	BASEPROOF_SOAK_ENTRIES               total entries to submit (default 1_000_000)
	BASEPROOF_SOAK_CONCURRENCY           concurrent submitters (default 8)
	BASEPROOF_SOAK_VERIFY_SAMPLES        sample of entries to /raw-check at the end.
	...
	BASEPROOF_SOAK_TREE_PROOF_SAMPLES    sample of inclusion proofs to verify against
	                                   /v1/tree/head root via merkle/proof. Same shape
	                                   as VERIFY_SAMPLES. When unset, cascades from
	                                   BASEPROOF_SOAK_VERIFY_SAMPLES (then default 100).
	BASEPROOF_SOAK_SMT_PROOF_SAMPLES     sample of SMT membership proofs to verify against
	                                   /v1/smt/root via SDK proof verifier. Same shape
	                                   as VERIFY_SAMPLES. When unset, cascades from
	                                   BASEPROOF_SOAK_VERIFY_SAMPLES (then default 100).
	BASEPROOF_SOAK_P99_BOUND_MS          HTTP admission p99 ceiling, ms (default 100)
	BASEPROOF_SOAK_SHIPPER_MAX_IN_FLIGHT shipper worker pool size (default 16)
	                                   ...
	                                   the budget.
	BASEPROOF_SOAK_DRAIN_TIMEOUT         in-test wait for WAL HWM to reach
...
BASEPROOF_SOAK_SHIPPER_MAX_IN_FLIGHT=64 + BASEPROOF_SOAK_DRAIN_TIMEOUT=30m.
*/
package tests
...
type soakLedger struct {
	...
	// WitnessEnabled is true when the builder loop's cosigner was
	// DISCOVERED from the LEDGER_WITNESS_* env (quorum active); false in
	// NON-WITNESS mode. CosignedCheckpointPath is where a successful
	// K-of-N collection publishes the cosigned-checkpoint file.
	WitnessEnabled         bool
...
func startSoakLedger(t *testing.T) *soakLedger {
	...
	if dsn == "" {
		t.Skip("BASEPROOF_TEST_DSN not set — skipping soak test")
	}
	...
	// BASEPROOF_TEST_S3_* env vars and the bucket from
	// BASEPROOF_TEST_S3_BUCKET (instead of BASEPROOF_TEST_GCS_BUCKET).
	backendType := os.Getenv("BASEPROOF_SOAK_BYTESTORE_BACKEND")
	if backendType == "" {
		backendType = "gcs"
	}
	...
	switch backendType {
	case "gcs":
		bucket = os.Getenv("BASEPROOF_TEST_GCS_BUCKET")
		if bucket == "" {
			t.Skip("BASEPROOF_TEST_GCS_BUCKET not set — skipping soak test")
		}
	case "s3":
		bucket = os.Getenv("BASEPROOF_TEST_S3_BUCKET")
		if bucket == "" {
			t.Skip("BASEPROOF_TEST_S3_BUCKET not set — skipping soak test")
		}
	default:
		t.Fatalf("BASEPROOF_SOAK_BYTESTORE_BACKEND=%q unsupported (gcs|s3)", backendType)
	}
	...
	if backendType == "s3" {
		bsConfig.S3Endpoint = os.Getenv("BASEPROOF_TEST_S3_ENDPOINT")
		bsConfig.S3AccessKey = os.Getenv("BASEPROOF_TEST_S3_ACCESS_KEY")
		bsConfig.S3SecretKey = os.Getenv("BASEPROOF_TEST_S3_SECRET_KEY")
		bsConfig.S3Region = os.Getenv("BASEPROOF_TEST_S3_REGION")
		...
		// real AWS, set BASEPROOF_TEST_S3_PATH_STYLE=false.
	...
	// Real Tessera in t.TempDir(). The builder loop (run below)
	// cosigns each checkpoint via the witness tier DISCOVERED from the
	// LEDGER_WITNESS_* env (cosignerFromEnv): production-shape K-of-N
	// quorum when those vars point at an already-running fleet,
	// NON-WITNESS (a logged warning) otherwise. The soak never spawns
	// or orchestrates witnesses itself.
	soakHarness := newTesseraHarness(t, ctx, logger)
	witnessCosignerHS, witnessEnabled := cosignerFromEnv(t, pool, logger)
	var witnessCosigner opbuilder.WitnessCosigner
	if witnessEnabled {
		witnessCosigner = witnessCosignerHS
	}
	...
	// De-pollution: builder computes over the TailedNodeStore, not a PG node
	// store. No reconciler runs here, so the tail holds every node (in-memory).
	nodeStore := store.NewTailedNodeStore(smt.NewInMemoryNodeStore())
	smtTree := smt.NewTree(leafStore, nodeStore)
	...
	// MaxInFlight (BASEPROOF_SOAK_SHIPPER_MAX_IN_FLIGHT). Without this
	...
	seqMaxInFlight := envInt("BASEPROOF_SOAK_SEQUENCER_MAX_IN_FLIGHT", 0)
	...
	// BASEPROOF_SOAK_SHIPPER_MAX_IN_FLIGHT for high-volume soaks where
	// real-GCS upload latency × workers is the drain bottleneck.
	maxInFlight := envInt("BASEPROOF_SOAK_SHIPPER_MAX_IN_FLIGHT", 16)
	...
	// append/commit) lands in the test output. The shared `logger` is
	// WARN-level to keep the WAL / sequencer / shipper output quiet during
	// the long admission burst; the builder's "batch processed" line is THE
	// signal we read to attribute per-batch latency to a stage.
	builderLogger := slog.New(slog.NewTextHandler(os.Stderr,
		&slog.HandlerOptions{Level: slog.LevelInfo}))
	builderLoop := opbuilder.NewBuilderLoop(
		loopCfg, pool, smtTree, leafStore, nodeStore,
		cursorReader, fetcher, nil, deltaBuffer, bufferStore, commitPub,
		soakHarness.Adapter, builderLogger,
	).WithRootStore(rootStateStore)

	// CheckpointLoop — cosigns + publishes the horizon over the durable committed
	// root, lagging the builder (production wiring). The soak asserts on the
	// cosigned-checkpoint file (soakHarness.CosignedCheckpointPath), so it runs
	// whenever a witness cosigner is configured.
	if witnessCosigner != nil {
		checkpointLoop := opbuilder.NewCheckpointLoop(
			store.NewSMTCommitCursor(rootStateStore),
			store.NewPgTileFrontier(pool),
			store.NewBuildTilesEmitter(nodeStore, store.NewPosixSMTTileStore(soakHarness.TileRoot)),
			soakHarness.Adapter, soakHarness.Adapter,
			witnessCosigner,
	...
	op := &soakLedger{
		...
		CosignedCheckpointPath: soakHarness.CosignedCheckpointPath(),
	}

	// Single ordered teardown — see tests/shutdownchain_test.go.
	// BASEPROOF_SOAK_KEEP_DATA=1 preserves the populated entry_index
	// + bytestore objects after the test exits.
	cleanFn := func() {
		if os.Getenv("BASEPROOF_SOAK_KEEP_DATA") != "" {
			t.Logf("BASEPROOF_SOAK_KEEP_DATA set — skipping cleanTables. " +
...
// 1M entries but BASEPROOF_SOAK_ENTRIES tunes it down to a smoke run
...
func TestSoak_LedgerBytestore(t *testing.T) {
	...
	total := envInt("BASEPROOF_SOAK_ENTRIES", 1_000_000)
	concurrency := envInt("BASEPROOF_SOAK_CONCURRENCY", 8)
	verifySamples := envSampleCount("BASEPROOF_SOAK_VERIFY_SAMPLES", 100, uint64(total))
	p99BoundMs := envInt("BASEPROOF_SOAK_P99_BOUND_MS", 100)
	...
	maxInFlightLog := envInt("BASEPROOF_SOAK_SHIPPER_MAX_IN_FLIGHT", 16)
	seqMaxInFlightLog := envInt("BASEPROOF_SOAK_SEQUENCER_MAX_IN_FLIGHT", 0)
	drainTimeoutLog := envDuration("BASEPROOF_SOAK_DRAIN_TIMEOUT", 10*time.Minute)
	...
	// of what BASEPROOF_SOAK_VERIFY_SAMPLES asked for, with no
	...
	for w := 0; w < concurrency; w++ {
		...
		go func(workerID int) {
			...
			for i := 0; i < per; i++ {
				...
				// (baseproof/builder/algorithm.go:50-63): one SMT mutation
	...
	// BASEPROOF_SOAK_DRAIN_TIMEOUT (any time.ParseDuration value).
	drainTimeout := envDuration("BASEPROOF_SOAK_DRAIN_TIMEOUT", 10*time.Minute)
	drainStart := time.Now()
	cycle := 0
	for {
		cycle++
		...
		// counts (1M+) crank BASEPROOF_SOAK_DRAIN_TIMEOUT to match the
	...
	// ── QUORUM EVIDENCE ──────────────────────────────────────────────
	// When the witness tier was DISCOVERED from the env, the builder
	// must have collected a K-of-N cosignature and published the
	// cosigned-checkpoint file by the time it caught up to expectedHWM
	// (the drain break gate above requires builderReady). Its absence
	// means the quorum path silently no-op'd against the configured
	// fleet — a regression the non-witness fallback would otherwise hide.
	if op.WitnessEnabled {
		ckpt := op.CosignedCheckpointPath
		if _, statErr := os.Stat(ckpt); statErr != nil {
			t.Errorf("WITNESS: quorum enabled but no cosigned-checkpoint at %s after drain: %v",
				ckpt, statErr)
		} else {
			t.Logf("WITNESS: quorum verified — cosigned-checkpoint present at %s", ckpt)
		}
	}
	...
	//                         BASEPROOF_SOAK_P99_BOUND_MS. Re-printed
	...
	admissionTPSFloor := envFloat("BASEPROOF_SOAK_ADMISSION_TPS_FLOOR", 100.0)
	drainTPSFloor := envFloat("BASEPROOF_SOAK_DRAIN_TPS_FLOOR", 50.0)
	maxFailureRate := envFloat("BASEPROOF_SOAK_MAX_FAILURE_RATE", 0.001)
	...
	// BASEPROOF_SOAK_VERIFY_SAMPLES asked for, and the test reported
	...
	if len(results) < verifySamples {
		t.Fatalf("verify-pool UNDERFILL: BASEPROOF_SOAK_VERIFY_SAMPLES requested %d but only "+
...
// per-verifier sample knobs so a single BASEPROOF_SOAK_VERIFY_SAMPLES
...
// Resolution order:
//
//  1. name (e.g., BASEPROOF_SOAK_TREE_PROOF_SAMPLES) — explicit
//     per-verifier override, intuitive shape.
//  2. cascade (e.g., BASEPROOF_SOAK_VERIFY_SAMPLES) — global default
...
// max=1.0 means "any failure rate"); use BASEPROOF_SOAK_*=0 for
...
//
// Sample size N is BASEPROOF_SOAK_TREE_PROOF_SAMPLES (default 100).
...
func verifyTreeIntegrity(t *testing.T, op *soakLedger, submitted uint64) {
	...
	// Cascades from BASEPROOF_SOAK_VERIFY_SAMPLES so a single global
	...
	n := envSampleCountCascade(
		"BASEPROOF_SOAK_TREE_PROOF_SAMPLES",
		"BASEPROOF_SOAK_VERIFY_SAMPLES",
		100, submitted)
	if n <= 0 {
		t.Logf("verifyTreeIntegrity: BASEPROOF_SOAK_TREE_PROOF_SAMPLES=%d → skipping proof verification", n)
		return
	}
...
//
// Sample size N is BASEPROOF_SOAK_SMT_PROOF_SAMPLES (default 100).
...
func verifySMTConsistency(t *testing.T, op *soakLedger, submitted uint64) {
	...
	// proofs) override via BASEPROOF_SOAK_SMT_PROOF_SAMPLES.
	n := envSampleCountCascade(
		"BASEPROOF_SOAK_SMT_PROOF_SAMPLES",
		"BASEPROOF_SOAK_VERIFY_SAMPLES",
		100, submitted)
	if n <= 0 {
		t.Logf("verifySMTConsistency: BASEPROOF_SOAK_SMT_PROOF_SAMPLES=%d → skipping", n)
		return
	}
	if uint64(n) > submitted {
		n = int(submitted)
...
func verifyEvidenceFetchAll(t *testing.T, ctx context.Context, op *soakLedger, submitted uint64) {
	...
	// with BASEPROOF_SOAK_KEEP_DATA=1 has an exact URL to curl post-
...
func logScalingEvidence(t *testing.T, op *soakLedger) {
	...
	// failures (see BASEPROOF_SOAK_P99_BOUND_MS) un-diagnosable.
	if op.WAL != nil {
