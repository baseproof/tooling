...
type Config struct {
	...
	// Tessera integration-batcher cadence. Zero values fall back to
	// tessera.AppenderOptions.applyDefaults. See cmd/ledger.Config for
...
func allocateTelemetry(cfg Config, d *deps.AppDeps) error {
	...
	d.GossipMeter = mp.Provider.Meter("github.com/baseproof/tooling/services/ledger/gossip")
...
func allocateTessera(ctx context.Context, cfg Config, signers SignerLoader, d *deps.AppDeps) error {
	...
	// leak telemetry (baseproof_tessera_close_drain_residual_goroutines
	...
	embedded, err := tessera.NewEmbeddedAppender(ctx, driver, tessera.AppenderOptions{
		...
		Antispam:             d.Antispam,
		// Integration-batcher cadence. A short BatchMaxAge keeps
		// AppendLeaf latency (and therefore committed throughput at a
		// given SequencerMaxInFlight) bounded; zero values fall back to
		// AppenderOptions.applyDefaults.
		CheckpointInterval: cfg.TesseraCheckpointInterval,
		BatchSize:          cfg.TesseraBatchSize,
		BatchMaxAge:        cfg.TesseraBatchMaxAge,
	}, d.Logger)
	if err != nil {
