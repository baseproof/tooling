...
|---|---|---|---|
| `baseproof_api_errors_total` | counter | `error_class`, `http_status` | `api/errors.go:81` |
| `baseproof_api_request_duration_seconds` | histogram | `route` (currently `"*"`) | `api/instruments.go:71` |

### Sequencer / shipper (`sequencer/`, `shipper/`)

| Metric | Type | Labels | Source |
|---|---|---|---|
| `baseproof_sequencer_drain_lag_seconds` | gauge | — | `sequencer/instruments.go:58` |
| `baseproof_shipper_pending_total` | gauge | — | `shipper/instruments.go:47` |
| `baseproof_shipper_shipped_total` | counter | — | `shipper/instruments_counters.go:107` |
| `baseproof_shipper_shipped_unique_total` | counter | — | `shipper/instruments_counters.go:115` |
| `baseproof_shipper_skipped_inflight_total` | counter | — | `shipper/instruments_counters.go:123` |
| `baseproof_shipper_retries_total` | counter | — | `shipper/instruments_counters.go:131` |
| `baseproof_shipper_mark_failures_total` | counter | — | `shipper/instruments_counters.go:139` |

### Storage primitives (`wal/`, `tessera/`, `bytestore/`, `store/`)

| Metric | Type | Source |
|---|---|---|
| `baseproof_wal_submit_duration_seconds` | histogram | `wal/instruments.go:73` |
| `baseproof_tessera_append_duration_seconds` | histogram | `tessera/instruments.go:50` |
| `baseproof_bytestore_put_duration_seconds` | histogram | `bytestore/instruments.go:51` |
| `baseproof_postgres_pool_acquire_seconds` | histogram | `store/instruments.go:52` |
...
|---|---|---|---|
| `baseproof_gossip_received_total` | counter | `kind`, `outcome` | Inbound publish accepts vs. rejects, dimensioned by gossip Kind |
| `baseproof_gossip_published_total` | counter | `kind`, `peer`, `outcome` | Outbound publish per-peer fan-out |
| `baseproof_gossip_verify_duration_seconds` | histogram | `kind` | Verifier time per Kind |
| `baseproof_gossip_chain_head_lamport` | gauge | `originator` | Per-originator current chain head Lamport time |
| `baseproof_gossip_store_size_total` | gauge | `kind` | Total events in the local gossip store |
| `baseproof_gossip_panic_total` | counter | `panic_kind` | Panics recovered by the SDK handler's self-encapsulating defer |
| `baseproof_witness_quorum_failures_total` | counter | `network_id` | `gossipnet/instruments.go:53` |

### `baseproof_gossip_panic_total` panic_kind values
...
1. Increment `baseproof_api_errors_total{error_class, http_status}`
...
Total baseproof_api_errors_total
  time-series:                    ~330
```
...
sum by (error_class) (
  rate(baseproof_api_errors_total{error_class=~"signature_invalid|admission_proof_invalid|destination_mismatch"}[5m])
)

# Ledger infrastructure — page on any uptick
rate(baseproof_api_errors_total{error_class=~"wal_backpressure|wal_persist_failed|sct_signing_failed|db_query_failed|db_unavailable"}[1m])

# Tenant state — informational dashboard, not paging
rate(baseproof_api_errors_total{error_class=~"insufficient_credits|expired_session"}[5m])

# Any panic at all is page-worthy — code bug or hostile-input attack the SDK contained
sum by (panic_kind) (rate(baseproof_gossip_panic_total[5m])) > 0

# Per-peer gossip fan-out failure rate
sum by (peer) (rate(baseproof_gossip_published_total{outcome="failed"}[5m]))
```
