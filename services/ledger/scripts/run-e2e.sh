...
# (postgres + witness daemon) via docker compose,
...
DOCKER_DSN="postgres://baseproof:baseproof@localhost:5544/baseproof_test?sslmode=disable"
...
# Witness image: strict-pull the published tooling image (the ledger
# builds no witness and needs no witness source checkout). Override WITNESS_IMAGE
# to point at a local build for unpublished changes.
export WITNESS_IMAGE="${WITNESS_IMAGE:-ghcr.io/baseproof/tooling/witness:latest}"
echo "== witness image: ${WITNESS_IMAGE} =="

echo
echo "== pulling images + bringing up topology =="
...
for i in $(seq 1 60); do
    pg_state=$(docker inspect --format='{{.State.Health.Status}}' baseproof_e2e_postgres 2>/dev/null || echo "")
    wit_state=$(docker inspect --format='{{.State.Health.Status}}' baseproof_e2e_witness 2>/dev/null || echo "")
...
# ── Run e2e tests ────────────────────────────────────────────────
export BASEPROOF_TEST_DSN="${BASEPROOF_TEST_DSN:-${DOCKER_DSN}}"
