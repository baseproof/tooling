...
#
# Required env:
#   BASEPROOF_REAL_GCS_BUCKET   the bucket name (REQUIRED)
...
#
# Usage:
#   export BASEPROOF_REAL_GCS_BUCKET=my-test-bucket
...
# ── Validate inputs ───────────────────────────────────────────────
if [ -z "${BASEPROOF_REAL_GCS_BUCKET:-}" ]; then
    echo "FATAL: BASEPROOF_REAL_GCS_BUCKET not set"
    echo
    echo "  export BASEPROOF_REAL_GCS_BUCKET=<your-bucket-name>"
    echo "  $0"
    exit 1
fi
...
echo
echo "  gsutil ls 'gs://${BASEPROOF_REAL_GCS_BUCKET}/test/**' || echo '(no leftovers — good)'"
