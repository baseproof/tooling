...
// ─────────────────────────────────────────────────────────────────
// ParseAllowedPurposes — least-privilege flag vocabulary
// ─────────────────────────────────────────────────────────────────

func TestParseAllowedPurposes(t *testing.T) {
	cases := []struct {
		name    string
		csv     string
		want    []cosign.Purpose
		...
		{"default tree-head", "tree-head", []cosign.Purpose{cosign.PurposeTreeHead}, false},
		{"tree-head+rotation", "tree-head,rotation",
			[]cosign.Purpose{cosign.PurposeTreeHead, cosign.PurposeRotation}, false},
		{"whitespace tolerated", " tree-head , rotation , escrow-override ",
			[]cosign.Purpose{cosign.PurposeTreeHead, cosign.PurposeRotation, cosign.PurposeEscrowOverride}, false},
		{"empty rejected", "", nil, true},
		{"only commas rejected", " , ,", nil, true},
		{"unknown token rejected", "treehead", nil, true},
		{"partial-unknown rejected", "tree-head,bogus", nil, true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := ParseAllowedPurposes(tc.csv)
			if tc.wantErr {
				if err == nil {
					t.Fatalf("ParseAllowedPurposes(%q): expected error, got %v", tc.csv, got)
				}
				return
			}
			if err != nil {
				t.Fatalf("ParseAllowedPurposes(%q): %v", tc.csv, err)
			}
			if len(got) != len(tc.want) {
				t.Fatalf("ParseAllowedPurposes(%q) size = %d, want %d", tc.csv, len(got), len(tc.want))
			}
			for _, p := range tc.want {
				if _, ok := got[p]; !ok {
					t.Errorf("ParseAllowedPurposes(%q) missing %q", tc.csv, p)
				}
			}
		})
	}
...
// TestAllowedPurposes_TreeHeadOnly_RejectsRotation is the
// load-bearing least-privilege assertion: a handler built with the
// default {PurposeTreeHead} set 403s a (well-formed) rotation
// cosign request before it ever reaches the signer. Proves the
// nil-AllowedPurposes signing-oracle gap is closed.
func TestAllowedPurposes_TreeHeadOnly_RejectsRotation(t *testing.T) {
	netID := testNetID()
	allowed, err := ParseAllowedPurposes("tree-head")
	if err != nil {
		t.Fatalf("ParseAllowedPurposes: %v", err)
	}
	h, err := Build(Config{
		WitnessKey:      testKey(t),
		NetworkID:       netID,
	...
	// A well-formed rotation cosign request: PurposeRotation +
	// WireRotationPayload{new_set_hash}. The monotonicity guard
	// passes non-tree-head purposes through to the SDK handler,
	// which 403s because rotation is not in AllowedPurposes.
	newSetHash := [32]byte{0x9A, 0xBC, 0xDE, 0xF0}
	rotPayload, err := json.Marshal(cosign.WireRotationPayload{
		NewSetHash: hex.EncodeToString(newSetHash[:]),
	})
	if err != nil {
		t.Fatalf("marshal rotation payload: %v", err)
	}
	body, err := json.Marshal(cosign.WireRequest{
		Purpose:   cosign.PurposeRotation,
		Payload:   rotPayload,
	...
	if rec.Code != http.StatusForbidden {
		t.Errorf("rotation on tree-head-only witness: got %d (%s), want 403 Forbidden",
			rec.Code, rec.Body.String())
	}

	// Sanity: the same handler still accepts tree-head at the same size.
	if r := postCosign(t, h, netID, 1); r.Code != http.StatusOK {
		t.Errorf("tree-head on tree-head-only witness: got %d (%s), want 200",
...
func buildTreeHeadRequestSimple(t *testing.T, netID cosign.NetworkID, size uint64) []byte {
	...
	// SMTRoot was added to the wire payload in baseproof v0.7.0 and
	...
	// baseproof v1.9.0; the SDK's DecodeWirePayload now requires a
	// 32-byte receipt_root (zero hash is the valid empty-batch
	// sentinel). A non-zero value here keeps the fixture realistic.
	receiptRoot := [32]byte{0x55, 0x66, 0x77, 0x88, byte(size)}
