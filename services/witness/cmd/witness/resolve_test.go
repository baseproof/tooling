...
// The injection-resolution contract: an explicit value (flag/env) wins
// VERBATIM, else the first existing candidate (std mount path, then the PaaS
// /etc/secrets twin), else "".

func TestResolveFile_ExplicitWinsVerbatim(t *testing.T) {
	dir := t.TempDir()
	existing := filepath.Join(dir, "witness.pem")
	if err := os.WriteFile(existing, []byte("x"), 0o600); err != nil {
		t.Fatal(err)
	}
	if got := resolveFile("/absent/explicit.pem", existing); got != "/absent/explicit.pem" {
...
func TestResolveFile_CandidateOrder(t *testing.T) {
	dir := t.TempDir()
	std := filepath.Join(dir, "std", "witness.pem")
	paas := filepath.Join(dir, "secrets", "witness.pem")
...
func TestPortAddrOr(t *testing.T) {
	t.Setenv("PORT", "10000")
	if got := portAddrOr(":8081"); got != ":10000" {
		t.Fatalf("PORT set ⇒ :$PORT; got %q", got)
	}
	t.Setenv("PORT", "")
	if got := portAddrOr(":8081"); got != ":8081" {
