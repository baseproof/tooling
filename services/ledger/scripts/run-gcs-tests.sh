...
for i in $(seq 1 60); do
    ...
    health="$(docker inspect --format='{{.State.Health.Status}}' baseproof_test_gcs 2>/dev/null || true)"
...
export BASEPROOF_TEST_GCS_ENDPOINT='http://127.0.0.1:4443/storage/v1/'
export BASEPROOF_TEST_GCS_BUCKET='baseproof-test-bytes'

echo "== ensuring test bucket exists in fake-gcs =="
# fake-gcs-server's bucket-init container creates "baseproof-tiles"
# (used by Tessera's tile writer). The byte-store tests use a
# different bucket name (baseproof-test-bytes) so they don't clash
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" \
    ...
    -d "{\"name\":\"${BASEPROOF_TEST_GCS_BUCKET}\"}" \
    "http://127.0.0.1:4443/storage/v1/b" || true)
case "${HTTP_CODE}" in
    200|201|409)
        echo "test bucket '${BASEPROOF_TEST_GCS_BUCKET}' ready (status=${HTTP_CODE})"
        ;;
    *)
