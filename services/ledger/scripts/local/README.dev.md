...
`scripts/run-local.sh` are generated by `cmd/genesis-ceremony dev`.
...
| `postgres` | `:5432` | Shared Postgres 18 with two databases (`baseproof_node_a`, `baseproof_node_b`) |
...
| `/healthz` returns 503 from `ledger-node-b` | `baseproof_node_b` database doesn't exist | `make dev-down && make dev-up` (full reset; init script only runs on fresh volumes). |
