...
// t.Skip'd when BASEPROOF_TEST_DSN is unset, so the package
...
func TestReconstruct_MatchesPersistedRoot(t *testing.T) {
	...
	const numLeaves = 50
	leaves := buildSyntheticLeaves(t, numLeaves, "did:baseproof:log:test")
...
func TestReconstruct_DetectsTampering(t *testing.T) {
	...
	const numLeaves = 30
	leaves := buildSyntheticLeaves(t, numLeaves, "did:baseproof:log:tamper")
	expectedRoot := computeExpectedRoot(t, leaves)
	populateSMTState(t, pool, leaves, expectedRoot, int64(numLeaves-1))
	...
	tamperedAuthTip := store.SerializeLogPosition(types.LogPosition{
		LogDID:   "did:baseproof:log:tamper",
...
func openTestPG(t *testing.T) (*pgxpool.Pool, func()) {
	t.Helper()
	adminDSN := os.Getenv("BASEPROOF_TEST_DSN")
	if adminDSN == "" {
		t.Skip("BASEPROOF_TEST_DSN not set — reconstructor PG tests need a Postgres instance")
	}
