...
func (s *stubSubmissionWAL) Submit(ctx context.Context, hash [32]byte, wire []byte, logTimeMicros int64, _ []types.Web3VerificationReceipt) error {
	if s.submitErr != nil {
		return s.submitErr
	}
...
func makeSubmissionDeps(t *testing.T, opSignerPriv *ecdsa.PrivateKey, signerPub *ecdsa.PublicKey, walFake *stubSubmissionWAL) *SubmissionDeps {
	...
	// Post-II #3: tests run unauthenticated Mode-B submissions;
	// wire the resolver + flag so the gate's capability + intent
	// are both present (preserves pre-refactor behaviour where
	// PoW was unconditional).
	diffResolver, _ := admission.NewStaticDifficultyResolver(diffController)
