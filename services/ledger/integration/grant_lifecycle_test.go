...
Skip semantics:
  ...
  - Skips when BASEPROOF_TEST_DSN is unset. Local developers can opt
    in by exporting BASEPROOF_TEST_DSN to a disposable Postgres
    database.
...
import (
	...
	opapi "github.com/baseproof/tooling/services/ledger/api"
...
// Skips when BASEPROOF_TEST_DSN is unset.
func requireDB(t *testing.T) *pgxpool.Pool {
	t.Helper()
	dsn := os.Getenv("BASEPROOF_TEST_DSN")
	if dsn == "" {
		t.Skip("BASEPROOF_TEST_DSN unset; integration suite skipped")
	}
