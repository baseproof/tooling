...
or github.com/baseproof/tooling/services/ledger/store.
...
type CosignedTreeHead struct {
	...
	// ReceiptRoot is the Merkle root over the batch's
	// Web3VerificationReceipts at the same TreeSize, bound into the
	// witness-cosigned payload as the third root (SDK v1.9.1's
	// 104-byte RootHash || SMTRoot || ReceiptRoot || TreeSize). Zero
	// array for an empty-receipt batch (smt.ReceiptRoot(nil)); the
	// cosign payload Validate exempts a zero ReceiptRoot. Persisted
	// so a head reconstructed from PG recomputes the exact canonical
	// message the witnesses signed.
	ReceiptRoot [32]byte
...
type CommitmentRow struct {
	...
	PostSMTRoot   [32]byte
	// MutationsCID addresses the off-log mutation blob in the artifact store
	// (storage.SMTDerivationCommitmentRef.MutationsCID, "sha256:hex"). Replaces
	// the former inline MutationsJSON — the #190 fix.
	MutationsCID  string
...
func (c ErrorClass) String() string {
	...
	case ErrorClassWriteAuthRejected:
		return "write_auth_rejected"
	case ErrorClassHorizonUnavailable:
...
const (
	...
	ErrorClassWriteAuthRejected          // 403 — gate 5 (write-path authorization)

	// 5xx — SDK uniform-verify gate fetch / IO failure
	ErrorClassEvidenceChainUnavailable // 500 — PR-F (fetcher fault)

	// 503 — the witness-cosigned horizon (published cosigned checkpoint) is
	// not yet available (fresh network / pre-genesis). The read front has
	// nothing trust-rooted to anchor an as-of SMT proof on; retry after the
	// first checkpoint publishes.
	ErrorClassHorizonUnavailable
)
