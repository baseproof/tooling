...
type AppenderOptions struct {
	...
	// "baseproof-local-dev" when empty — fine for tests, never
	// for production.
	Origin string
...
type EmbeddedAppender struct {
	// upstream resources from tessera.NewAppender — held to drive Add and Head.
	appender *uptessera.Appender
	reader   uptessera.LogReader

	// teardown performs the complete, correct shutdown for whichever Tessera is
	// linked. It is resolved ONCE at construction by fingerprinting the appender
	// (see resolveTeardown): the fork's deterministic drain-and-join, or
	// upstream's drain + cancel + best-effort drainBudget wait. Close simply calls
	// it, so there is no fork/upstream branching anywhere else in this type.
	teardown func(ctx context.Context) error

	// deterministic is the fingerprint result: true iff the baseproof fork
	// (transparency-dev#341) is linked, so teardown joins the background
	// goroutines rather than best-effort waiting. Surfaced via
	// DeterministicShutdown for logs and tests.
	deterministic bool
...
func NewEmbeddedAppender(
	...
	// Derive a private context for upstream's background goroutines that is
	// DECOUPLED from the caller's cancellation (context.WithoutCancel). This is
	// load-bearing: the caller's ctx is the process signal context, and the
	// checkpoint publisher is one of the goroutines bound here. If bgCtx were a
	// child of ctx, SIGTERM would kill the publisher at signal time — before the
	// teardown chain reaches Close — and the drain in Close could never advance
	// the checkpoint (stranding the last entries). By isolating bgCtx, Close is
	// the sole authoritative teardown for Tessera: the publisher stays alive so
	// the drain completes, then we stop the goroutines. The teardown chain
	// always invokes Close, so nothing leaks. See Close for the fork/upstream
	// split.
	...
	// Fingerprint the linked Tessera ONCE and resolve a single teardown strategy.
	// Everything downstream (Close, tests) reads the resulting capability instead
	// of re-probing or branching.
	teardown, deterministic := resolveTeardown(appender, shutdown, bgCancel, opts.DrainBudget)
	...
	logger.Info("tessera embedded appender ready",
		...
		"deterministic_shutdown", deterministic,
	)
...
// forkShutdowner is the fingerprint of the baseproof/tessera fork: its
// *Appender implements Shutdown(ctx) — a deterministic drain-and-join teardown
// (transparency-dev#341) — whereas Google upstream's *Appender does not. The
// interface is named (not inline) so the capability is documented in exactly one
// place, and it compiles against BOTH Tessera builds because it only names a
// method signature. If upstream later adds the same method, it is picked up
// automatically — the fingerprint is the capability, not a vanity marker.
type forkShutdowner interface {
	Shutdown(ctx context.Context) error
}

// resolveTeardown fingerprints appender and returns the single teardown function
// to run at Close, plus whether the deterministic (fork) path was selected.
//
// This is the ONLY place the fork and upstream are distinguished. Both returned
// closures share the same shape — drain, then ensure the background goroutines
// are gone — so Close needs no knowledge of which Tessera is linked.
func resolveTeardown(
	appender *uptessera.Appender,
	drain func(ctx context.Context) error,
	bgCancel context.CancelFunc,
	drainBudget time.Duration,
) (func(ctx context.Context) error, bool) {
	if fork, ok := any(appender).(forkShutdowner); ok {
		// Fork: Shutdown drains in-flight Adds AND stops + joins every background
		// goroutine, blocking until they return. No drainBudget guesswork.
		return func(ctx context.Context) error {
			err := fork.Shutdown(ctx)
			bgCancel() // release our derived bg-ctx (idempotent: goroutines joined)
			return err
		}, true
	}
	// Upstream: the returned function only drains in-flight Adds; we then cancel
	// the goroutines' context and best-effort wait drainBudget for them to exit
	// (upstream exposes no join). bgCtx is decoupled from the caller's signal
	// context, so the publisher is alive for the drain (see NewEmbeddedAppender).
	return func(ctx context.Context) error {
		err := drain(ctx)
		bgCancel()
		if drainBudget > 0 {
...
// DeterministicShutdown reports whether Close performs a deterministic
// drain-and-join (baseproof/tessera fork) rather than a best-effort
// drain + drain-budget wait (Google upstream). It is the resolved fingerprint,
// exposed for logging, metrics, and tests.
func (e *EmbeddedAppender) DeterministicShutdown() bool { return e.deterministic }
...
func (e *EmbeddedAppender) AppendLeaf(ctx context.Context, data []byte) (uint64, error) {
	...
	ctx, span := otel.Tracer("github.com/baseproof/tooling/services/ledger/tessera").Start(ctx, "tessera.AppendLeaf")
	defer span.End()
	t0 := time.Now() // D3
...
// IntegratedSize delegates to the upstream LogReader's IntegratedSize,
// satisfying the AppenderBackend contract for tile-derived size-bound
// root resolution (TesseraAdapter.RootAtSize). Per the upstream
// docstring, this value is the current size of the integrated tree —
// it MAY be ahead of the latest published checkpoint, and is safe to
// use only for log-internal processes (the builder loop is one).
func (e *EmbeddedAppender) IntegratedSize(ctx context.Context) (uint64, error) {
	return e.reader.IntegratedSize(ctx)
}
...
func (e *EmbeddedAppender) PublishCosignedCheckpoint(
	...
	return publishCheckpoint(e.publicCheckpointPath, head)
}
...
// publishCheckpoint makes the per-size archive copy durable, THEN writes the
// LOAD-BEARING latest checkpoint (the horizon). Extracted from the appender so the
// fail-closed contract is unit-testable.
//
// Durable-before-horizon (Phase 1, symmetric with store/horizon_s3.go): the latest
// cosigned-checkpoint is the horizon — the single object every as-of proof anchors to
// — and a cold/PG-off reader resolves a covering checkpoint from the per-size archive
// (1.1a). So the archive must be durable BEFORE the horizon advertises this size: the
// archive is written first and an archive error WITHHOLDS the horizon (fail-closed),
// exactly as the S3 publisher does, since a cold reader has no PG fallback to degrade
// to. A withheld horizon self-heals on the next publish; history that predates this
// archiver is regenerated by the backfill (recovery.ArchiveBackfill / G1).
func publishCheckpoint(publicPath string, head types.CosignedTreeHead) error {
	body, err := json.Marshal(types.FromCosignedTreeHead(head))
	if err != nil {
		return fmt.Errorf("tessera/embedded: marshal cosigned head: %w", err)
	}
	// Archive FIRST, fail-closed: the horizon must never advance past a size whose
	// per-size archive did not become durable, or a cold/PG-off read for that size
	// would miss its covering checkpoint. Skipped at size 0 (pre-genesis).
	if head.TreeSize > 0 {
		if aerr := archiveCheckpoint(publicPath, head.TreeSize, body); aerr != nil {
			return fmt.Errorf("tessera/embedded: archive checkpoint %d not durable, withholding horizon: %w", head.TreeSize, aerr)
		}
	}
	// Horizon LAST: advances only once the per-size archive is durable.
...
// Its error is LOAD-BEARING — publishCheckpoint WITHHOLDS the horizon on it
// (fail-closed), so the per-size archive is durable before the horizon advertises
// this size.
...
func (e *EmbeddedAppender) Close(ctx context.Context) error {
	e.closeOnce.Do(func() {
		// One call, no version branching: teardown was resolved by the fingerprint
		// at construction. Fork → drain + cancel + join (deterministic); upstream →
		// drain + cancel + best-effort drainBudget wait.
		e.closeErr = e.teardown(ctx)

		// Leak diagnostic — coarse but actionable. On the deterministic (fork)
		// path this should read zero (goroutines were joined); a persistent
		// non-zero delta on the upstream path means its background goroutines
		// didn't drain within budget. Wired to OTel via recordCloseDrainResidual.
		residual := runtime.NumGoroutine() - e.goroutineBaseline
		if residual > 0 {
			e.logger.Warn("tessera embedded: goroutines did not drain to baseline",
				"deterministic_shutdown", e.deterministic,
				...
				"hint", "on upstream Tessera, NewAppender spawns Follower/followerStats/updateStats with no join; a non-zero residual under drain budget means these are still running. Link the baseproof fork for a deterministic join.")
		}
...
// getSignerOrGenerate("baseproof-local-dev") helper, lifted here
...
// IntegratedSize on a read-only adapter is the latest signed
// checkpoint's TreeSize — the conservative bound a non-writer can
// observe. Read-only consumers cannot see in-flight tile state past
// the published checkpoint, and that is exactly the right bound for
// the CT-aligned size-bound resolution contract.
func (r *ReadOnlyAppender) IntegratedSize(ctx context.Context) (uint64, error) {
	head, err := r.Head()
	if err != nil {
		return 0, err
	}
