...
database. BASEPROOF_TEST_DSN is therefore not required, and these
...
func TestAdmission_RejectsInsufficientWitnessQuorum(t *testing.T) {
	// A valid ECDSA witness key: real secp256k1 point, canonical
	// sha256(pubkey) ID, declared SchemeTag — baseproof v1.14.0's
	// NewWitnessKeySet rejects anything less. The crypto is never
	// exercised here; the set only needs to construct so the
	// empty-signatures rejection path can run.
	witScalar := make([]byte, 32)
	witScalar[31] = 1
	witPriv, err := signatures.PrivKeyFromBytes(witScalar)
	if err != nil {
		t.Fatalf("PrivKeyFromBytes: %v", err)
	}
	witPub := signatures.PubKeyBytes(&witPriv.PublicKey)
	witKey := types.WitnessPublicKey{
		ID:        sha256.Sum256(witPub),
		PublicKey: witPub,
