...
DESCRIPTION:
	...
	round-trips. Opt-in via:
	    ...
	    BASEPROOF_TEST_GCS_BUCKET=ledger-tile-integration-<your-instance> \
...
// server (BASEPROOF_TEST_GCS_ENDPOINT set). The build tag already
// gates the file, but a developer can have BASEPROOF_TEST_GCS_ENDPOINT
...
func requireRealGCS(t *testing.T) {
	...
	if os.Getenv("BASEPROOF_TEST_GCS_ENDPOINT") != "" {
		t.Skip("BASEPROOF_TEST_GCS_ENDPOINT set — these tests require REAL GCS, not fake-gcs-server")
	}
	if os.Getenv("BASEPROOF_TEST_GCS_BUCKET") == "" {
		t.Skip("BASEPROOF_TEST_GCS_BUCKET unset — real-GCS integration tests require a bucket")
	}
}
