...
gossipnet's EquivocationMonitor + EquivocationPublisher:
 ...
 1. EquivocationMonitorConfig accepts WitnessKeys — a
    WitnessKeySetProvider yielding a single *cosign.WitnessKeySet (the
    shared quorum.Manager) — no separate QuorumK / NetworkID /
    BLSVerifier args. NewEquivocationMonitor surfaces a typed rejection
    for a nil WitnessKeys so boot wiring's failure mode is visible at
    startup, not at first tick.
...
// ecdsaWitnessKey returns a valid ECDSA WitnessPublicKey derived
// deterministically from seed: a real secp256k1 point, a canonical
...
// synthetic 4-byte form is no longer constructible. Distinct seeds
// yield distinct keys.
func ecdsaWitnessKey(t *testing.T, seed byte) types.WitnessPublicKey {
	t.Helper()
	scalar := make([]byte, 32)
	scalar[31] = seed
	priv, err := signatures.PrivKeyFromBytes(scalar)
	if err != nil {
		t.Fatalf("PrivKeyFromBytes(seed=%d): %v", seed, err)
	}
...
func TestV011_EquivocationMonitor_AcceptsSingleKeyset(t *testing.T) {
	...
	m, err := NewEquivocationMonitor(EquivocationMonitorConfig{
		...
		WitnessKeys: quorum.NewManager(set),
...
func TestV011_EquivocationMonitor_RejectsNilKeyset(t *testing.T) {
	...
	if err == nil {
		t.Fatal("nil WitnessKeys accepted; want explicit boot failure")
	}
}
...
func TestV011_DetectEquivocation_ReadsKFromSet(t *testing.T) {
	...
	witKey := ecdsaWitnessKey(t, 0x88)
	set, err := sdkcosign.NewWitnessKeySet(
		[]types.WitnessPublicKey{witKey, ecdsaWitnessKey(t, 0x99)},
		nonZeroNetworkID(),
		2, // K=2 of 2
		nil,
	)
