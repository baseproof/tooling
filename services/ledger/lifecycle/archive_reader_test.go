...
// fixtureReader returns a fresh ArchiveReader wired with an
// SDK-DefaultClient — the same posture production wiring uses, so
// the RetryAfterRoundTripper round-trip pinned by 503→200 tests
// behaves identically here.
func fixtureReader(t *testing.T) *ArchiveReader {
	t.Helper()
	return NewArchiveReader(nil, sdklog.DefaultClient(30*time.Second, nil))
}
