...
instance (BASEPROOF_TEST_DSN). The ledger subprocess connects to
...
ENV VAR CONTRACT
	...
	BASEPROOF_TEST_DSN must point at a Postgres instance the test
...
type Postgres struct {
	...
	// (postgres or baseproof_test). Used to issue DROP DATABASE
...
func NewPostgres(t *testing.T) *Postgres {
	...
	if adminDSN == "" {
		t.Skip("BASEPROOF_TEST_DSN not set — chaos harness needs a Postgres instance")
	}
	...
	defer func() { _ = adminConn.Close(adminCtx) }()
