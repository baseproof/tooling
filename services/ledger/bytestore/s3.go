...
// PutObject writes data at key verbatim (no entries prefix), for substrate
// stores that own their key layout — e.g. content-addressed SMT tiles keyed by
// smt.TilePath. Idempotent for content-addressed callers (identical bytes per
// key, last-writer-wins). Keeps the SMT-tile substrate on the SAME S3-compatible
// backend (SeaweedFS in this deployment) as the entry byte store — no second
// storage dependency, no local filesystem.
...
// GetObject reads the bytes at key, returning ErrNotFound when the object is
// absent so callers can map to their own sentinel (e.g. os.ErrNotExist, which
// smt.TiledNodeStore treats as a clean absent node).
...
// HeadObject reports whether an object exists at key — a cheap existence check
// with no body transfer (S3 HEAD). NotFound maps to (false, nil); any other
// error surfaces. Used as the reconciler's exact tile-existence `known` oracle.
