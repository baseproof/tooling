...
Run: BASEPROOF_TEST_DSN="postgres://..." go test ./tests/ -v -count=1 -run TestHTTP
*/
package tests
...
func buildModeBWireEntry(t *testing.T, header envelope.ControlHeader, payload []byte, logDID string, difficulty uint32) []byte {
	...
	// once muEnableDestinationBound is on (baseproof v1.0.0+).
...
func TestHTTP_Submission_ModeB_WrongLogDID_403(t *testing.T) {
	...
	}, []byte("wrong-log-payload"), "did:baseproof:different-log", 16)
