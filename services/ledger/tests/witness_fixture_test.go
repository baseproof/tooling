...
func newWitnessFixture(t *testing.T, netID cosign.NetworkID, n int) *witnessFixture {
	...
	for i := 0; i < n; i++ {
		...
		// Canonical uncompressed secp256k1 bytes (0x04||X||Y) — the
		// same form witness.KeysFromDIDs produces in production and
		// the form NewWitnessKeySet binds the ID to (ID must equal
		// sha256(PublicKey), which signer.PubKeyID() already is).
