...
type AppenderBackend interface {
	...
	// IntegratedSize returns the inclusive upper bound on tree sizes
	// whose RFC 6962 root can be derived deterministically from
	// tiles right now. Used by TesseraAdapter.RootAtSize as the
	// durability gate for size-bound cosign-root resolution — the
	// CT-aligned discipline that decouples the builder's cosign
	// payload from the sequencer's wall-clock head (issue #189
	// follow-on; this addition is the additive PR-1 surface, no
	// behavior change yet).
	//
	// IntegratedSize MAY be ahead of the latest signed checkpoint
	// (Head().TreeSize): leaves can be integrated into the tile
	// substrate before a checkpoint covering them is published.
	// Per the upstream tessera contract, this value is safe to
	// consult ONLY for processes internal to the operation of the
	// log (the builder loop is such a process); external consumers
	// MUST anchor on Head().
