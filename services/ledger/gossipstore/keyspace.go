...
0x01..0x06 reserved by wal/keyspace.go) without colliding.
	...
	0x07 0x0E <binding:32>                              → SignedEvent JSON (history-rewrite projection)
	0x07 0x0F <binding:32>                              → SignedEvent JSON (smt-replay projection)
...
const (
	...
	// subHistoryRewriteProj is the read-side projection of verified
	// BP-GOSSIP-LEDGER-HISTORY-REWRITE-V1 findings (SDK plan §I.2 / §I.13).
	// Mirrors the 0x0B equivocation-projection shape — keyed by the
	// finding's content-derived binding hash so the read endpoint
	// computes the key identically and serves a point lookup.
	//
	//   Key:   0x07 0x0E <binding:32>
	//   Value: SignedEvent JSON (the verified gossip event carrying
	//          the history-rewrite proof)
	//
	// Written by:
	//   - A monitor / publisher that locally detects a history
	//     rewrite (cross-TreeSize append-only violation) and emits
	//     the gossip event. Detector wiring to the monitor is a
	//     follow-up; the keyspace is allocated here so that work
	//     does not need to re-number.
	//   - The gossip handler's post-Append hook when an inbound
	//     KindHistoryRewriteFinding event verifies — peer-detected
	//     history rewrites also surface on this network's /by-binding
	//     endpoint.
	//
	// CONTRASTS WITH 0x0B (equivocation, same-size) and 0x0F (smt-
	// replay, same-size + same-RootHash + different SMTRoot). The
	// three projections are KEYED BY DISTINCT prefixes precisely
	// because the read endpoints partition the audit pipeline by
	// offence class — history-rewrite alerts route to the
	// append-only-violation dashboard, smt-replay to the
	// state-derivation dashboard, equivocation to the same-size
	// fork dashboard.
	subHistoryRewriteProj byte = 0x0E

	// subSMTReplayProj is the read-side projection of verified
	// BP-GOSSIP-LEDGER-SMT-REPLAY-V1 findings (SDK plan §I.2 / §I.13).
	// Same shape as 0x0E — keyed by the finding's binding hash.
	//
	//   Key:   0x07 0x0F <binding:32>
	//   Value: SignedEvent JSON (the verified gossip event carrying
	//          the SMT-replay proof)
	//
	// Written by:
	//   - The equivocation monitor's SMTReplayPublisher branch when
	//     it locally detects same-size + same-RootHash + different
	//     SMTRoot across the peer + local heads.
	//   - The gossip handler's post-Append hook when an inbound
	//     KindSMTReplayFinding event verifies.
	subSMTReplayProj byte = 0x0F
)
