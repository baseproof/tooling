...
type AntiEntropyConfig struct {
	...
	// gossip pulls. REQUIRED — nil is rejected at construction (see
	// baseproof v1.34 CHANGELOG: the SDK no longer manufactures a
	// plaintext default). Production callers should pass
	// sdklog.DefaultClient(20*time.Second, tlsCfg) for the
	// RetryAfterRoundTripper transparent 503 retry posture.
...
func NewAntiEntropy(cfg AntiEntropyConfig) (*AntiEntropy, error) {
	...
	// error in a caller that constructed AntiEntropyConfig{} by
	// hand. Fail-closed.
	if cfg.HTTPClient == nil {
		return nil, fmt.Errorf("gossipnet/antientropy: HTTPClient required (pass cfg.HTTPClient — the SDK no longer manufactures a plaintext default)")
	}
