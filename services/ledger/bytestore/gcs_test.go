...
vars are unset, mirroring the BASEPROOF_TEST_DSN skip pattern in
...
naming so tests and prod stay in sync):
	...
	BASEPROOF_TEST_GCS_ENDPOINT e.g. http://localhost:4443/storage/v1/
	BASEPROOF_TEST_GCS_BUCKET e.g. baseproof-test-bytes
...
// vars are set:
//
//	BASEPROOF_TEST_GCS_ENDPOINT set → fake-gcs mode (anonymous=true)
//	BASEPROOF_TEST_GCS_BUCKET set → real GCS mode (ADC)
//	neither set → t.Skip
//
...
func requireGCS(t *testing.T) *GCS {
	...
	endpoint := os.Getenv("BASEPROOF_TEST_GCS_ENDPOINT")
	bucket := os.Getenv("BASEPROOF_TEST_GCS_BUCKET")

	if endpoint == "" && bucket == "" {
		t.Skip("neither BASEPROOF_TEST_GCS_ENDPOINT nor BASEPROOF_TEST_GCS_BUCKET set; skipping GCS test")
	}

	fakeMode := endpoint != ""
	if !fakeMode && bucket == "" {
		t.Skip("BASEPROOF_TEST_GCS_BUCKET unset for real-GCS mode; skipping")
	}
	if fakeMode && bucket == "" {
...
func TestGCS_DifferentObjectPrefix_IsolatesData(t *testing.T) {
	...
	bucket := os.Getenv("BASEPROOF_TEST_GCS_BUCKET")
	if endpoint == "" && bucket == "" {
		t.Skip("neither BASEPROOF_TEST_GCS_ENDPOINT nor BASEPROOF_TEST_GCS_BUCKET set")
	}
	fakeMode := endpoint != ""
	if fakeMode && bucket == "" {
		bucket = "baseproof-test-bytes"
