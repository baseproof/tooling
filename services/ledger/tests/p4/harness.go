...
// requirePostgres returns a connected pool against BASEPROOF_TEST_DSN
...
func requirePostgres(t *testing.T) *pgxpool.Pool {
	...
	if dsn == "" {
		t.Skip("BASEPROOF_TEST_DSN unset; skipping P4 test (use scripts/run-p4.sh)")
	}
...
func freshPool(t *testing.T) *pgxpool.Pool {
	...
	dsn := os.Getenv("BASEPROOF_TEST_DSN")
	if dsn == "" {
		t.Skip("BASEPROOF_TEST_DSN unset; skipping P4 test")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	pool, err := pgxpool.New(ctx, dsn)
