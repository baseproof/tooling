...
```
$ go list -m github.com/baseproof/baseproof
github.com/baseproof/baseproof v0.1.3
...
ok  21 packages with _test.go (admission, anchor, api, api/middleware,
   ...
   gossipnet, gossipstore, integration (skips without BASEPROOF_TEST_DSN),
...
`go doc github.com/baseproof/baseproof/crypto/sct SignSCT`:
> doc: no symbol SignSCT in package …/crypto/sct

The SDK package header explicitly notes: *"The ledger (which
holds the private key) retains SignSCT in baseproof-ledger/api/
...
1. Skips when `BASEPROOF_TEST_GCS_BUCKET` is unset.
2. **Refuses** fake-gcs (fails the test if
   `BASEPROOF_TEST_GCS_ENDPOINT` is set). Integration tests must
...
fake-gcs option (`BASEPROOF_TEST_GCS_ENDPOINT`) so developers can
...
1. Bump the pin in `go.mod` —
   `go get github.com/baseproof/baseproof@vX.Y.Z`.
2. Run `go vet ./...` — catches signature drift at the 19
   interface anchors.
3. Run `go test -count=1 -race -short ./...` — every package's
   pinning tests run.
4. Run the integration suite (`BASEPROOF_TEST_DSN` + ADC creds
