...
func runMatrixCell(
	...
	t.Run(cellName, func(t *testing.T) {
		...
		head := types.TreeHead{
			...
			// SMTRoot MUST be non-zero: the SDK witness handler
			// rejects a cosign request whose smt_root is all-zero
			// ("dual-commitment binding requires both roots
			// populated"). The production builder always sets it;
			// an all-zero head would fail at request validation —
			// before the K-of-N path runs — making the offline
			// cells pass for the wrong reason.
			SMTRoot: [32]byte{0xCC, 0xDD, byte(totalN), byte(quorumK), byte(online)},
		}
...
// returned CosignedTreeHead carries between K and N signatures:
// at least K (quorum was reached) and at most N (can't collect
// more sigs than there are witnesses). The collector fires all N
// requests concurrently and cancels once K have landed, so the
// final count may overshoot K when several responses arrive in
// the same window — hence a [K, N] range, not exactly K. The
// Strict-STH-Finality CDN write must serialize the same K-of-N
// evidence the gossip publisher transmits.
func TestP4_WitnessMatrix_SignaturesUnderQuorum(t *testing.T) {
	...
	head := types.TreeHead{
		...
		SMTRoot:  [32]byte{0x5A, 0x7C}, // non-zero: dual-commitment binding (see runMatrixCell)
	}
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
