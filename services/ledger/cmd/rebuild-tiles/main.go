...
USAGE:
	...
	# 2. Back up Postgres.
	pg_dump baseproof > /var/backups/ledger-pre-rebuild-$(date -I).sql
...
func main() {
	...
	// ── Stores ────────────────────────────────────────────────────────
	// De-pollution: the Jellyfish node DAG is no longer persisted in PG. This
	// one-shot rebuild computes the SMT over an in-memory node substrate purely
	// to drive the builder loop's Tessera appends + smt_leaves writes; the
	// durable SMT tiles are (re)emitted from smt_leaves by the runtime
	// reconciler, not here.
	...
	// NewBuilderLoop takes the TailedNodeStore substrate. Here it wraps a fresh
	// in-memory node store: the rebuild's nodes are an ephemeral computation
	// artifact (root = f(smt_leaves)), discarded at exit. The durable SMT tiles
	// are rebuilt from smt_leaves by the runtime reconciler. For very large logs
	// this in-memory tail grows with tree size — run rebuild-tiles with adequate
	// memory, or boot the ledger (whose reconciler tiles + prunes) afterward.
	tailed := store.NewTailedNodeStore(smt.NewInMemoryNodeStore())
	tree := smt.NewTree(leafStore, tailed)
