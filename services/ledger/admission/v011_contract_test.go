...
Three load-bearing properties of the v0.1.1 break:
 ...
 1. *cosign.WitnessKeySet is the single boot-time topology object —
    ...
    The admission BLSQuorumVerifier reads ONE such object from a
    WitnessKeySetProvider (the shared quorum.Manager); the previous
    (keys, K, networkID, blsVerifier) parameter group + the
