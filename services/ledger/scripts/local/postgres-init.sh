...
#   baseproof_node_a   (default; owned by ledger-node-a; created
#                     by the postgres image from POSTGRES_DB)
#   baseproof_node_b   (owned by ledger-node-b; created here)
#
...
psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER}" --dbname "${POSTGRES_DB}" <<-EOSQL
    CREATE DATABASE baseproof_node_b;
    GRANT ALL PRIVILEGES ON DATABASE baseproof_node_b TO ${POSTGRES_USER};
EOSQL

echo "postgres-init: created baseproof_node_b database"
