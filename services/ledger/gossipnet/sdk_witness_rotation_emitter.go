...
v0.7.0 baseproof SDK release that ships findings.WitnessRotation
...
func (e *SDKGossipWitnessRotationEmitter) Emit(
	...
	// v1.39: the rotation FACT is carried inside the on-log entry bytes;
	// decode it once for the diagnostic log fields below. The finding
	// passed NewWitnessRotationFinding's Validate at the call site, so a
	// decode error here is not expected; the emit proceeds regardless
	// (rot feeds only diagnostics, not the signed/broadcast bytes).
	rot, _ := finding.Rotation()
	...
	if err := e.cfg.Sink.Broadcast(ctx, signed); err != nil {
		...
		e.log.WarnContext(ctx,
			...
			"effective_pos", finding.EffectivePos.String(),
		)
		return
	}
