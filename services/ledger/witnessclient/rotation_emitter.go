...
KindWitnessRotation (shipped in baseproof v0.6.0; cryptographic
...
func (e *LoggingWitnessRotationEmitter) Emit(ctx context.Context, f *findings.WitnessRotationFinding) {
	...
	e.emitted.Add(1)
	// v1.39: the rotation FACT lives inside the on-log entry bytes; decode
	// it for the diagnostic line. A finding that fails to decode could
	// never have passed NewWitnessRotationFinding's Validate, so this is
	// defensive only.
	rot, err := f.Rotation()
	if err != nil {
		e.logger.ErrorContext(ctx,
			"witnessclient: rotation event has undecodable entry (gossip-disabled deployment; logging only)",
			"effective_pos", f.EffectivePos.String(),
			"err", err,
		)
		return
	}
	e.logger.InfoContext(ctx,
		...
		"current_set_hash", rot.CurrentSetHash[:8],
		"new_set_size", len(rot.NewSet),
		"scheme_tag_old", rot.SchemeTagOld,
		"scheme_tag_new", rot.SchemeTagNew,
		"current_sigs", len(rot.CurrentSignatures),
		"new_sigs", len(rot.NewSignatures),
		"effective_pos", f.EffectivePos.String(),
