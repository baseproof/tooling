...
func Rebuild(ctx context.Context, deps RebuildDeps) (Stats, error) {
	...
	// De-pollution: the Jellyfish node DAG is no longer persisted in PG. Nodes are
	// an ephemeral derivation (root = f(smt_leaves)); an in-memory node store holds
	// the tree across batches so each ProcessBatch reads the siblings the prior
	// batch produced. The rebuild restores smt_leaves + smt_root_state (below); the
	// durable SMT tiles are rebuilt from smt_leaves by the runtime reconciler.
	nodeStore := smt.NewInMemoryNodeStore()
	...
	for batchStart := uint64(0); batchStart < treeSize; batchStart += uint64(deps.BatchSize) {
		...
		// Per-batch overlay tree wrapped over the persistent leafStore + in-memory
		// nodeStore. The overlay captures every leaf and every dirty Jellyfish node
		// ProcessBatch produces; pre-commit failures (PG error etc.) discard the
		// overlays and leave the persistent stores untouched. On commit success the
		// leaf mutations persist in the atomic transaction (entry_index + cursor),
		// and the dirty nodes are flushed to the in-memory node store afterward.
		...
		// ephemeral derivation flushed to the in-memory node store after the commit.
		...
		// Flush this batch's dirty nodes into the (in-memory) node store so the next
		// batch's overlay + mainTree resolve them as committed siblings. A pre-commit
		// failure returns above, leaving these unflushed — consistent with the leaves.
		for _, node := range dirtyNodes {
			if _, pErr := nodeStore.Put(node); pErr != nil {
				return Stats{}, fmt.Errorf("rebuild: node flush [%d, %d): %w", batchStart, batchEnd, pErr)
			}
		}
