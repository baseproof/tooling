...
The word "commitment" is overloaded across two distinct concepts:
  ...
  - DERIVATION commitments (this file): ledger-published commentary
    ...
    mutations_cid, mutation_count) tuple for a builder batch. The mutation
    set itself lives off-log in the artifact store, addressed by
    mutations_cid (the #190 fix); this index stores the CID, not the bulk.
    Used by fraud-proof verifiers to attest that a specific run of the
    builder against a specific entry range produced a specific SMT
    mutation set.
	...
	_, err := s.db.Exec(ctx, `
		INSERT INTO derivation_commitments
			(range_start_seq, range_end_seq, prior_smt_root, post_smt_root,
			 mutations_cid, mutation_count, commentary_seq)
		VALUES ($1, $2, $3, $4, $5, $6, $7)`,
		row.RangeStartSeq, row.RangeEndSeq,
		row.PriorSMTRoot[:], row.PostSMTRoot[:],
		row.MutationsCID, int64(row.MutationCount), row.CommentarySeq,
	)
	if err != nil {
...
func (s *CommitmentStore) QueryBySequence(ctx context.Context, seq uint64) (*CommitmentRow, error) {
	...
	err := s.db.QueryRow(ctx, `
		SELECT id, range_start_seq, range_end_seq, prior_smt_root, post_smt_root,
		       mutations_cid, mutation_count, commentary_seq, created_at
		...
		&priorRoot, &postRoot,
		&row.MutationsCID, &mutationCount, &row.CommentarySeq, &row.CreatedAt)
	...
	row.MutationCount = uint32(mutationCount)
	return &row, nil
}
