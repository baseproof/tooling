...
func (s *Sequencer) processOne(ctx context.Context, hash [32]byte) {
	...
	// PR-N4: rehydrate the per-signature Web3VerificationReceipts
	// from the WAL Meta record (persisted by api/submission Step 11
	// via wal.Committer.Submit). The slice flows in TWO directions
	// from this point on:
	//
	//   (a) Onto the stagedEntry.Web3Receipts field, for any future
	//       in-process consumer (currently unused).
	//   (b) Onto tuple.Row.Web3ReceiptsBytes — the serialized form
	//       persisted by store.EntryStore.InsertBatch into
	//       entry_index.web3_receipts (migration 0009). The builder
	//       reads it back via PostgresEntryFetcher.Fetch and
	//       deserializes into types.EntryWithMetadata.Web3Receipts
	//       for the per-batch ReceiptRoot computation (PR-N5).
	//
	// Both paths converge on the same canonical wire encoding
	// (envelope.SerializeWeb3VerificationReceipt). nil/empty is the
	// normal shape for entries admitted via the legacy single-sig
	// path or under the (default-OFF) multi-sig gate.
	tuple.Web3Receipts = meta.Web3Receipts
	if len(meta.Web3Receipts) > 0 {
		encoded, encErr := store.EncodeEntryWeb3Receipts(meta.Web3Receipts)
		if encErr != nil {
			// Serialization failure on already-validated SDK-emitted
			// receipts is a programmer error (Validate ran inside the
			// SDK before they reached us); log + drop the receipts
			// rather than tombstone the entry. The on-chain receipt
			// is best-effort projection metadata, not consensus.
			s.logger.Error("sequencer: encode web3_receipts — dropping",
				"seq", seq, "hash", hashPrefix(hash), "error", encErr)
		} else {
			tuple.Row.Web3ReceiptsBytes = encoded
		}
	}
