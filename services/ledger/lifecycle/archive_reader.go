...
// httpClient is the *http.Client used for every shard fetch — pass
// d.OutboundHTTPClient (mTLS-equipped, if the deployment configures
// peer mTLS) so archive reads carry the same TLS posture as anchor /
// witness / gossip outbound calls. REQUIRED — nil panics. As of
// the v1.34 SDK contract, downstream SDK constructors reject a nil
// *http.Client at construction time; this constructor matches the
// posture so a forgotten boot wire surfaces as a panic at process
// start rather than a successful boot with broken outbound posture.
// Production wiring uses sdklog.DefaultClient(30s, tlsCfg) — see
// cmd/ledger/boot/wire/wire.go.
func NewArchiveReader(shards []ShardMeta, httpClient *http.Client) *ArchiveReader {
	if httpClient == nil {
		panic("lifecycle/archive_reader: httpClient required (the v1.34 SDK contract is no silent fallback)")
	}
...
// httpClient is the *http.Client used when source is an http(s):// URL —
// pass the same client the surrounding archive reader uses so all archive
// I/O carries one TLS posture. REQUIRED when source is an http(s):// URL;
// nil returns an error rather than falling back to a plaintext default
// (v1.34 SDK contract). File-path sources ignore httpClient.
func LoadShardIndex(ctx context.Context, source string, httpClient *http.Client) ([]ShardMeta, error) {
	var data []byte
	var err error

	if len(source) > 4 && (source[:4] == "http") {
		if httpClient == nil {
			return nil, fmt.Errorf("lifecycle/archive_reader: httpClient required for http(s) source (the v1.34 SDK contract is no silent fallback)")
		}
