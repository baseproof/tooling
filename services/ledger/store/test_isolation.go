...
(BASEPROOF_TEST_DSN). Default `go test ./...` runs packages in
...
//
// Skips when BASEPROOF_TEST_DSN is unset (same convention as
...
func IsolatedDB(t *testing.T) *pgxpool.Pool {
	...
	if dsn == "" {
		t.Skip("BASEPROOF_TEST_DSN unset; skipping PG-backed test")
	}
	...
	if _, execErr := rootPool.Exec(ctx,
		fmt.Sprintf("CREATE SCHEMA %q", schema),
	); execErr != nil {
		rootPool.Close()
		t.Fatalf("IsolatedDB: CREATE SCHEMA %q: %v", schema, execErr)
	}
