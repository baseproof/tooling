...
//
// Semantic (see store/credits.go): a deduction proceeds iff the
// pre-deduction balance is > 0; the deduction MAY take the balance
// negative (a 200-credit caller passing cost=256 is honored once),
// after which subsequent submissions are rejected until top-up.
//
// Used by:
//   - api/submission.go (deductCreditModeA, cost=1 per entry)
//   - api/batch.go (deductCreditModeA, cost=N once per batch)
//
// Returns apitypes.ErrInsufficientCredits when the pre-deduction
// balance is already <= 0; the api handler maps that to HTTP 402.
type CreditDeducter interface {
	Deduct(ctx context.Context, exchangeDID string, cost int64) error
}
