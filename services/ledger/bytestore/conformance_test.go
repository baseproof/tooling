...
skips):
	...
	TestConformance_GCS_Container (BASEPROOF_TEST_GCS_ENDPOINT)  Store
	TestConformance_GCS_Real (BASEPROOF_TEST_GCS_BUCKET, no
	                                 endpoint) Backend
	TestConformance_S3_Container (BASEPROOF_TEST_S3_ENDPOINT)   Backend
	                                (SeaweedFS / RustFS / MinIO with
	                                anonymous-read bucket policy)
	TestConformance_S3_Real (BASEPROOF_TEST_S3_REAL=1 +
	                                 BASEPROOF_TEST_S3_BUCKET)     Backend
...
func TestConformance_GCS_Container(t *testing.T) {
	...
	if os.Getenv("BASEPROOF_TEST_GCS_ENDPOINT") == "" {
		t.Skip("BASEPROOF_TEST_GCS_ENDPOINT unset; skipping fake-gcs conformance")
	}
	store := requireGCS(t)
...
func TestConformance_GCS_Real(t *testing.T) {
	...
	if os.Getenv("BASEPROOF_TEST_GCS_ENDPOINT") != "" {
		t.Skip("BASEPROOF_TEST_GCS_ENDPOINT set — that's container mode; this test is real-GCS only")
	}
	if os.Getenv("BASEPROOF_TEST_GCS_BUCKET") == "" {
		t.Skip("BASEPROOF_TEST_GCS_BUCKET unset; skipping real-GCS conformance")
	}
	store := requireGCS(t)
...
func TestConformance_S3_Container(t *testing.T) {
	...
	// verifiable URLs.
	if os.Getenv("BASEPROOF_TEST_S3_ENDPOINT") == "" {
		t.Skip("BASEPROOF_TEST_S3_ENDPOINT unset; skipping S3 container conformance")
	}
	if os.Getenv("BASEPROOF_TEST_S3_REAL") == "1" {
		t.Skip("BASEPROOF_TEST_S3_REAL=1 — that's real-AWS mode; this test is for the local container")
	}
	store := requireS3(t)
	runBackendConformance(context.Background(), t, store)
}
...
func TestConformance_S3_Real(t *testing.T) {
	...
	if os.Getenv("BASEPROOF_TEST_S3_REAL") != "1" {
		t.Skip("BASEPROOF_TEST_S3_REAL!=1; skipping real-AWS conformance")
	}
	if os.Getenv("BASEPROOF_TEST_S3_BUCKET") == "" {
		t.Skip("BASEPROOF_TEST_S3_BUCKET unset; skipping real-AWS conformance")
	}
	store := requireS3(t)
	runBackendConformance(context.Background(), t, store)
}
