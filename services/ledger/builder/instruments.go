...
var cosignSkippedTileLagState struct {
	mu      sync.RWMutex
	counter metric.Int64Counter
}

// InstallWitnessQuorumFailureCounter wires
// baseproof_witness_quorum_failures_total from an OTel meter. Returns
...
func InstallWitnessQuorumFailureCounter(meter metric.Meter) bool {
	...
	c, err := meter.Int64Counter(
		"baseproof_witness_quorum_failures_total",
...
// InstallCosignSkippedTileLagCounter wires
// baseproof_builder_cosigns_skipped_tile_lag_total — incremented when
// Step 6 (size-bound path) skips the witness cosignature because the
// durable tile substrate has not caught up to lastAppendedIdx+1. The
// SMT commit still happens; the next cycle's cosign covers the
// deferred SMT contribution. A sustained climb here means the tile
// emitter / object-store backend is the throughput bottleneck —
// escalate to capacity rather than treating it as a correctness alarm.
//
// Idempotent; nil meter → no-op.
func InstallCosignSkippedTileLagCounter(meter metric.Meter) bool {
	if meter == nil {
		return false
	}
	cosignSkippedTileLagState.mu.Lock()
	defer cosignSkippedTileLagState.mu.Unlock()
	if cosignSkippedTileLagState.counter != nil {
		return false
	}
	c, err := meter.Int64Counter(
		"baseproof_builder_cosigns_skipped_tile_lag_total",
		metric.WithDescription("Builder cycles whose witness cosignature was skipped because durable tile substrate lagged the batch's last appended index (size-bound Step 6 safety valve; SMT state still committed)."),
	)
	if err != nil {
		return false
	}
	cosignSkippedTileLagState.counter = c
	return true
}

// incCosignSkippedTileLag increments the skipped-cosign counter;
// safe to call before InstallCosignSkippedTileLagCounter (no-op).
func incCosignSkippedTileLag(ctx context.Context) {
	cosignSkippedTileLagState.mu.RLock()
	c := cosignSkippedTileLagState.counter
	cosignSkippedTileLagState.mu.RUnlock()
	if c == nil {
		return
	}
	c.Add(ctx, 1)
