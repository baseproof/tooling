...
Usage:
	...
	go run ./cmd/submit-stamp \
	    ...
	    -log-did "did:baseproof:ledger:001" \
	    -payload "hello world"
...
// hc is the outbound HTTP client used for every POST /v1/entries.
// ALWAYS retryhttp-backed for startup-race resilience (matches the
// peer CLIs — backfill, audit, admission-authority); the mTLS flags
// compose into the client when set, the retry posture is invariant
...
func main() {
	var (
		...
		logDID     = flag.String("log-did", "did:baseproof:ledger:001", "destination log DID (admission rejects mismatched Header.Destination)")
	...
	if err != nil {
		log.Fatalf("submit-stamp: mTLS config: %v", err)
	}
