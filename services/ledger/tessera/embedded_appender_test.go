...
func TestGenerateEphemeralSigner_DefaultsToBaseproof(t *testing.T) {
	signer, _, err := GenerateEphemeralSigner("")
	if err != nil {
		t.Fatalf("GenerateEphemeralSigner: %v", err)
	}
	if signer.Name() != "baseproof-local-dev" {
		t.Errorf("default signer name: got %q, want %q",
			signer.Name(), "baseproof-local-dev")
	}
}
...
// TestEmbeddedAppender_Close_DoneChannelClosesAfterDrainBudget pins the Done()
// channel contract: closed exactly once, after Close completes. Tests rely on
// this to gate t.TempDir cleanup until the background goroutines are gone.
//
// The drain-budget timing assertion is version-aware via the resolved
// fingerprint (DeterministicShutdown): upstream Close waits out the drain budget
// (it has no join), whereas the fork's Close returns as soon as it has joined and
// must NOT block for the budget.
func TestEmbeddedAppender_Close_DoneChannelClosesAfterDrainBudget(t *testing.T) {
	...
	// Timing depends on the fingerprinted teardown strategy.
	if app.DeterministicShutdown() {
		// Fork: Close joins the goroutines and returns immediately afterwards;
		// it must not sit out the drain budget. Allow generous headroom for the
		// join itself, but it should be well under the budget in this no-traffic
		// case.
		if closeElapsed >= drain {
			t.Errorf("deterministic Close took %s, expected < drainBudget=%s "+
				"(fork should join, not wait the budget)", closeElapsed, drain)
		}
	} else {
		// Upstream: no join, so Close waits out the drain budget.
		if closeElapsed < drain {
			t.Errorf("upstream Close returned in %s, expected >= drainBudget=%s "+
