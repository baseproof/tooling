...
Provides:
  ...
  - Postgres connection/migration helpers gated by BASEPROOF_TEST_DSN.
...
const testLogDID = "did:baseproof:test:integration"

// testEpochWindowSeconds is the epoch length used by HTTP integration tests.
// 1 hour matches the production default (BASEPROOF_EPOCH_WINDOW_SECONDS=3600)
...
func foreignPos(seq uint64) types.LogPosition {
	return types.LogPosition{LogDID: "did:baseproof:foreign", Sequence: seq}
}
...
// skipIfNoPostgres checks for BASEPROOF_TEST_DSN. Returns a pool or skips the test.
...
func connectPostgres(t *testing.T) *pgxpool.Pool {
	...
	if dsn == "" {
		t.Skip("BASEPROOF_TEST_DSN not set — skipping Postgres integration test")
	}
...
func cleanTables(t *testing.T, pool *pgxpool.Pool) {
	...
	ctx := context.Background()
	tables := []string{
		"tree_head_sigs", "entry_index", "smt_leaves",
	...
	// starts from a clean baseline matching an empty smt_leaves set.
	// The hex is derived dynamically from smt.EmptyHash so it tracks
	// the SDK across version bumps; in v0.3.0 this is sha256("") =
	// e3b0c4…b855 (the Jellyfish empty-tree root). A previous bug
	// hard-coded the v0.2.0 depth-256 constant here, which caused
	// every builder atomic commit to fail with "missing node" after
	// migration 0003 installed the v0.3.0 constant — leaf_count
	// stayed at 0.
