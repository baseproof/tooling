...
// testHTTPClient is the shared helper every NewAntiEntropy /
// NewEquivocationMonitor / sdkgossip.NewClient call in this
// package's tests uses. As of baseproof v1.34 the SDK + the
// ledger's gossipnet constructors reject a nil *http.Client
// (no silent fallback). 5s is plenty for the in-process
// httptest stubs this package uses.
...
// TestAntiEntropy_RejectsNilHTTPClient pins the v1.34 contract:
// NewAntiEntropy refuses to manufacture a default HTTP client when
// the caller omits one. Mirrors the SDK's fail-closed posture (see
// baseproof v1.34 CHANGELOG, gossip/cosign constructors).
func TestAntiEntropy_RejectsNilHTTPClient(t *testing.T) {
	store := sdkgossip.NewInMemoryStore()
	_, err := NewAntiEntropy(AntiEntropyConfig{
		Store: store,
		Peers: []AntiEntropyPeer{{DID: "did:web:peer", BaseURL: "https://peer.example"}},
		// HTTPClient deliberately omitted.
	})
	if err == nil {
