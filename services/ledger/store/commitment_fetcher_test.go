...
BASEPROOF_TEST_DSN is unset. The integration/ docker-compose harness
wires the env var so these tests run on every PR. Local developers
can run them by exporting BASEPROOF_TEST_DSN to a disposable Postgres
database.
*/
package store
...
func TestResetFixtures_LeavesNoRows(t *testing.T) {
	...
	preflightTables := []string{"entry_index", "commitment_split_id", "smt_leaves"}
