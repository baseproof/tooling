...
// ─────────────────────────────────────────────────────────────────────
// 0x0E — history-rewrite projection
// ─────────────────────────────────────────────────────────────────────

// PutHistoryRewriteProjection writes (or overwrites) a verified
// KindHistoryRewriteFinding event into the read-side projection
// (0x0E) keyed by binding hash. Mirrors PutEquivProjection's
// idempotency contract — re-receiving the same finding is a no-op
// (identical bytes overwrite identical bytes), preserving the
// gossip layer's I9 idempotency through the projection.
func (s *BadgerStore) PutHistoryRewriteProjection(
	ctx context.Context, binding [32]byte, signedEventBytes []byte,
) error {
	if err := ctx.Err(); err != nil {
		return err
	...
	if len(signedEventBytes) == 0 {
		return fmt.Errorf("gossipstore: PutHistoryRewriteProjection: empty signedEventBytes")
	}
	return s.db.Update(func(txn *badger.Txn) error {
		return txn.Set(historyRewriteProjKey(binding), signedEventBytes)
	})
}

// GetHistoryRewriteProjection returns the verified history-rewrite
// finding previously projected under the supplied binding hash, or
// (nil, nil) when no projection exists. Powers an audit/admin
// endpoint serving O(1) point reads on cross-size append-only
// violations without touching Postgres.
func (s *BadgerStore) GetHistoryRewriteProjection(
	ctx context.Context, binding [32]byte,
) ([]byte, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	...
	var out []byte
	err := s.db.View(func(txn *badger.Txn) error {
		item, gerr := txn.Get(historyRewriteProjKey(binding))
		if errors.Is(gerr, badger.ErrKeyNotFound) {
			return nil
		}
		if gerr != nil {
...
// ─────────────────────────────────────────────────────────────────────
// 0x0F — smt-replay projection
// ─────────────────────────────────────────────────────────────────────

// PutSMTReplayProjection writes (or overwrites) a verified
// KindSMTReplayFinding event into the read-side projection (0x0F)
// keyed by binding hash. Idempotency contract identical to
// PutEquivProjection / PutHistoryRewriteProjection.
func (s *BadgerStore) PutSMTReplayProjection(
	ctx context.Context, binding [32]byte, signedEventBytes []byte,
) error {
	if err := ctx.Err(); err != nil {
		return err
	...
	if len(signedEventBytes) == 0 {
		return fmt.Errorf("gossipstore: PutSMTReplayProjection: empty signedEventBytes")
	}
	return s.db.Update(func(txn *badger.Txn) error {
		return txn.Set(smtReplayProjKey(binding), signedEventBytes)
	})
}

// GetSMTReplayProjection returns the verified smt-replay finding
// previously projected under the supplied binding hash, or
// (nil, nil) when no projection exists. Powers an audit/admin
// endpoint serving O(1) point reads on SMT-derivation divergence
// without touching Postgres.
func (s *BadgerStore) GetSMTReplayProjection(
	ctx context.Context, binding [32]byte,
) ([]byte, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	...
	var out []byte
	err := s.db.View(func(txn *badger.Txn) error {
		item, gerr := txn.Get(smtReplayProjKey(binding))
		if errors.Is(gerr, badger.ErrKeyNotFound) {
			return nil
		}
		if gerr != nil {
