...
// packHashTile returns a level-0 HASH tile: `count` raw 32-byte leaf hashes
// concatenated with NO length prefix — the c2sp.org/tlog-tiles hash-tile
// format that TileReader.Fetch (level, index, p) actually serves, and the
// format the verifier consumes. (The prior fixture packed length-PREFIXED
// entry-bundle bytes, which matched the buggy parser instead of reality, so
// the test passed while production FATAL'd.) Returns the tile bytes plus the
// per-offset hashes so a test can assert HashAt slices the right one.
func packHashTile(t *testing.T, count int) ([]byte, [][32]byte) {
	t.Helper()
	out := make([]byte, 0, count*32)
	hashes := make([][32]byte, count)
	for i := 0; i < count; i++ {
		h := sha256.Sum256([]byte("leaf-" + string(rune('0'+i%10))))
		hashes[i] = h
		out = append(out, h[:]...)
	}
	return out, hashes
...
func TestVerifier_AsksForPartial_OffsetPlusOne(t *testing.T) {
	...
	tile, hashes := packHashTile(t, int(wantP))
	fetcher.put(0, wantP, tile)

	v := NewVerifier(fetcher.Fetch)

	got, err := v.HashAt(context.Background(), seq)
	if err != nil {
		t.Fatalf("HashAt(seq=%d): %v", seq, err)
	}
	if got != hashes[seq] {
		t.Errorf("HashAt(seq=%d) = %x, want %x (offset must slice the raw hash)", seq, got, hashes[seq])
	}
...
func TestVerifier_AsksForFullTile_WhenOffsetIs255(t *testing.T) {
	...
	fetcher := newRecordingFetcher()
	tile, _ := packHashTile(t, 256)
	fetcher.put(0, 0, tile)
...
func TestVerifier_PostBugFix_SecondTileIndex(t *testing.T) {
	...
	fetcher := newRecordingFetcher()
	tile, hashes := packHashTile(t, int(wantP))
	fetcher.put(wantIndex, wantP, tile)
	...
	if want := hashes[seq%EntriesPerEntryTile]; got != want {
		t.Errorf("HashAt(seq=%d) = %x, want %x", seq, got, want)
	}
...
// TestVerifier_HashTileFormat_RawHashes is the regression pin for the
// integrity FATAL on a healthy log: a level-0 hash tile is raw concatenated
// 32-byte leaf hashes, so HashAt must SLICE the offset, not run the
// length-prefixed entry-bundle parser (which read a hash's leading bytes —
// e.g. 0x211D = 8477 — as a length and reported "tile truncated", killing
// the ledger at the second entry). seq=1 → the SECOND raw hash in a 2-hash
// tile.
func TestVerifier_HashTileFormat_RawHashes(t *testing.T) {
	fetcher := newRecordingFetcher()
	tile, hashes := packHashTile(t, 2) // seq 0 + seq 1, 64 raw bytes
	fetcher.put(0, 2, tile)            // p = offset(1)+1 = 2

	v := NewVerifier(fetcher.Fetch)

	got, err := v.HashAt(context.Background(), 1)
	if err != nil {
		t.Fatalf("HashAt(seq=1) on a valid 2-hash tile must not error, got: %v", err)
	}
	if got != hashes[1] {
		t.Errorf("HashAt(seq=1) = %x, want %x", got, hashes[1])
	}
}

// TestVerifier_ShortHashTile_IsErrored pins genuine truncation: a tile too
// short to contain the requested offset's 32 bytes is a real error (the
// detector fail-closes), distinct from the format false-positive above.
func TestVerifier_ShortHashTile_IsErrored(t *testing.T) {
	fetcher := newRecordingFetcher()
	fetcher.put(0, 2, []byte{0x01, 0x02, 0x03}) // 3 bytes — cannot hold offset 1

	v := NewVerifier(fetcher.Fetch)

	if _, err := v.HashAt(context.Background(), 1); err == nil {
		t.Fatal("expected error on a genuinely short hash tile")
	}
}
