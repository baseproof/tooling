...
// reading baseproof v0.5.0 gossip/findings/ghost_leaf.go:Validate).
...
type Sequencer struct {
	...
	lagReader    LagReader
	replayer     *Replayer
	// recentCache is the in-process recent-entry cache (F1). Populated
	// from applyPostCommitForOne after each successful PG commit so the
	// builder's hot fetch path (PostgresEntryFetcher.Fetch) hits memory
	// instead of the bytestore. Nil-safe — wiring is optional via
	// WithRecentEntryCache. The cache is a pure optimization: the
	// durable path (entry_index + bytestore) is always correct.
	recentCache store.RecentEntryCache
	logDID      string
	cfg         Config
	logger      *slog.Logger
...
// WithRecentEntryCache wires the in-process recent-entry cache (F1).
// The sequencer Put's into the cache from applyPostCommitForOne after
// each successful PG commit; the SAME cache instance must be wired into
// store.PostgresEntryFetcher (via WithCache) so the builder's hot fetch
// path hits memory instead of the bytestore.
//
// Optional; nil cache is a no-op (the durable path stays correct without
// the cache — it is a pure optimization). Race-free against drain cycles
// only when called before Run starts. Tombstones and (in-batch / cross-
// batch) duplicates are skipped at the call site so the cache only ever
// holds entries the durable store already has.
func (s *Sequencer) WithRecentEntryCache(c store.RecentEntryCache) *Sequencer {
	s.recentCache = c
	return s
}
