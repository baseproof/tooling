...
// TreeSize=42 + non-zero RootHash + non-zero SMTRoot so that the
// byte serialization produced by cosign.NewTreeHeadPayload is not
// all-zeros (which might mask a serialization bug behind the
// cryptographic check) and passes the cosign dual-commitment
// Validate (all-zero SMTRoot is rejected on the sign path).
func representativeTreeHead() types.TreeHead {
	var root, smt [32]byte
	for i := range root {
		root[i] = byte(0x10 + i)
		smt[i] = byte(0x40 + i)
	}
	return types.TreeHead{TreeSize: 42, RootHash: root, SMTRoot: smt}
}
...
func TestCrossNetworkReplay_BLSQuorumVerifierRejects(t *testing.T) {
	...
	v := admission.NewBLSQuorumVerifier(quorum.NewManager(setB))
...
func TestCrossNetworkReplay_RoundTripControl(t *testing.T) {
	...
	v := admission.NewBLSQuorumVerifier(quorum.NewManager(setA))
	if err := v.VerifyEmbeddedTreeHead(signed); err != nil {
