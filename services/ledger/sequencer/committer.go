...
type stagedEntry struct {
	...
	// slice rehydrated from wal.Meta (PR-N3) and forwarded to the
	// builder (PR-N5) for per-batch ReceiptRoot computation. Index-
	// aligned with Entry.Signatures when non-empty. Nil/empty for
	// tombstones and for entries admitted via the legacy single-sig
	// path. The committer's projection writers (entry_index +
	// commitment_split_id) do not consume this field — it flows
	// THROUGH the committer to the builder via the sequencer's
	// post-commit hook.
	Web3Receipts []sdktypes.Web3VerificationReceipt
...
func (s *Sequencer) applyPostCommitForOne(ctx context.Context, e stagedEntry, dbCommitted bool) {
	...
	// Recent-entry cache (F1, in-process builder fast lane). Populated
	// for EVERY live entry (not just HasSplitID ones — that gate is only
	// for the cross-log canonical_hash projection above). Tombstones are
	// excluded by the early return at the top of this function; we
	// nil-check e.Entry defensively against any future tombstone-with-
	// Entry shape change. The cache holds *types.EntryWithMetadata
	// byte-identical to what the bytestore would return — Serialize is
	// a deterministic bijection on canonical envelopes, so the same
	// EntryIdentity (sha256 of canonical bytes) is preserved.
	if dbCommitted && !e.Tombstone && e.Entry != nil && s.recentCache != nil {
		canonical, serr := envelope.Serialize(e.Entry)
		if serr != nil {
			s.logger.Warn("sequencer: recent-entry cache Serialize failed",
				"seq", e.Seq, "error", serr)
		} else {
			// Deep-clone Web3Receipts at the cache boundary. stagedEntry
			// stays live in the committer (sidecar writes, post-commit
			// fan-out) after this Put, so aliasing the slices upward
			// would let a future contributor's `append` or in-place edit
			// silently corrupt cached EntryWithMetadata. The deep clone
			// touches:
			//   - the outer []Web3VerificationReceipt header
			//   - each receipt's Proof []byte
			//   - each receipt's ExecutorQuorum.Clients []ExecutorAttestation
			// Cost: typically 0-2 receipts × a handful of bytes — well
			// under a microsecond. Belt-and-suspenders against future
			// aliasing bugs, not a perf concern.
			s.recentCache.Put(e.Seq, &sdktypes.EntryWithMetadata{
				CanonicalBytes: canonical,
				LogTime:        e.Row.LogTime,
				Position:       sdktypes.LogPosition{LogDID: s.logDID, Sequence: e.Seq},
				Web3Receipts:   cloneWeb3Receipts(e.Web3Receipts),
			})
		}
	}
...
// cloneWeb3Receipts deep-copies a Web3VerificationReceipt slice so the
// returned slice shares no backing storage with src — neither the
// outer slice header nor the Proof bytes nor the ExecutorQuorum.Clients
// slice. Used at the recent-entry cache boundary where the source
// slice (stagedEntry.Web3Receipts) outlives the Put and could be
// re-aliased by a future contributor's `append` or in-place edit.
//
// nil → nil, empty → empty (preserves caller's "no Web3 verification
// happened" semantic). Note that ExecutorAttestation contains no
// nested slices (ClientID is string, ResultHash is [32]byte), so the
// inner Clients clone bottoms out at one level.
func cloneWeb3Receipts(src []sdktypes.Web3VerificationReceipt) []sdktypes.Web3VerificationReceipt {
	if src == nil {
		return nil
	}
	out := make([]sdktypes.Web3VerificationReceipt, len(src))
	for i, r := range src {
		// Copy the value (covers all fixed-size fields: ChainID,
		// BlockNumber, BlockHash, ContractAddr, ProofKind, K, N).
		out[i] = r
		// Deep-clone the byte slice Proof.
		out[i].Proof = slices.Clone(r.Proof)
		// Deep-clone the ExecutorQuorum.Clients slice. Each
		// ExecutorAttestation is a value type with no nested
		// slices, so a single-level Clone is sufficient.
		out[i].ExecutorQuorum.Clients = slices.Clone(r.ExecutorQuorum.Clients)
	}
	return out
}
