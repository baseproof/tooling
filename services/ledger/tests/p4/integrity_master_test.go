...
import (
	...
	upwitness "github.com/baseproof/baseproof/witness"
...
func TestP4_Integrity_SMT_RoundTrip(t *testing.T) {
	...
	// De-pollution: nodes are an in-process derivation; no PG node store.
...
func TestP4_Integrity_DetectEquivocation_FlagsFork(t *testing.T) {
	...
	// Same non-zero SMTRoot on both so RootHash is the sole
	// divergence (the fork). SMTRoot must be non-zero: cosign
	// Validate rejects an all-zero SMTRoot on the sign path.
	headA := types.TreeHead{
		TreeSize: 100,
		RootHash: [32]byte{0xAA},
		SMTRoot:  [32]byte{0x5A},
	}
...
func TestP4_Integrity_DetectEquivocation_AcceptsBenignDuplicate(t *testing.T) {
	...
	head := types.TreeHead{TreeSize: 200, RootHash: [32]byte{0xCC}, SMTRoot: [32]byte{0x5C}}
