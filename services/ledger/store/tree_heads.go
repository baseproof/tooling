...
//
// smtRoot and receiptRoot are the second and third roots bound
// into the witness-cosigned payload at this TreeSize (the 104-byte
// RootHash || SMTRoot || ReceiptRoot || TreeSize; baseproof SDK
// v0.8.0+ / v1.9.1). Callers pass the zero array when the log
// publishes no SMT projection / no Web3 receipts; the storage layer
// makes no semantic interpretation of the bytes. Persisting all
// three lets a head reconstructed from PG recompute the exact
// canonical message the witnesses signed.
func (s *TreeHeadStore) InsertHead(
	...
	_, err := s.db.Exec(ctx, `
		...
		treeSize, rootHash[:], smtRoot[:], receiptRoot[:], int16(hashAlgo),
	)
	if err != nil {
...
func (s *TreeHeadStore) LatestCosigned(ctx context.Context, minSigs int) (*CosignedTreeHead, error) {
	row := s.db.QueryRow(ctx, `
		SELECT h.tree_size, h.root_hash, h.smt_root, h.receipt_root, h.hash_algo, h.created_at
		FROM tree_heads h
		WHERE (
	...
	var rootHash, smtRoot, receiptRoot []byte
	err := row.Scan(&head.TreeSize, &rootHash, &smtRoot, &receiptRoot, &head.HashAlgo, &head.CreatedAt)
	...
	if len(receiptRoot) == 32 {
		copy(head.ReceiptRoot[:], receiptRoot)
	}
...
func (s *TreeHeadStore) GetBySize(ctx context.Context, size uint64) (*CosignedTreeHead, error) {
	...
	err := s.db.QueryRow(ctx, `
		SELECT tree_size, root_hash, smt_root, receipt_root, hash_algo, created_at
		FROM tree_heads
		WHERE tree_size = $1
		ORDER BY hash_algo ASC
		LIMIT 1`, size,
	).Scan(&head.TreeSize, &rootHash, &smtRoot, &receiptRoot, &head.HashAlgo, &head.CreatedAt)
