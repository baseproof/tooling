...
//     ("ORTHOLOG" in ASCII, MSB-first) so a refactor that silently
//     rotates the magic number doesn't split-brain a fleet that
...
// produces a fleet split-brain: old binaries hold lock 0xBASEPROOF,
// new binaries acquire lock 0xWHATEVER, both think they're the
// sole writer.
func TestP4_AdvisoryLock_LockIDInvariant(t *testing.T) {
	const want = int64(0x4F5254484F4C4F47) // "ORTHOLOG" in ASCII (MSB-first)
