...
func TestWAL_Submit_RejectsEmptyWire(t *testing.T) {
	...
	if err := c.Submit(ctx, hash, nil, time.Now().UnixMicro(), nil); !errors.Is(err, ErrEmptyWire) {
		t.Fatalf("expected ErrEmptyWire on nil, got %v", err)
	}
	if err := c.Submit(ctx, hash, []byte{}, time.Now().UnixMicro(), nil); !errors.Is(err, ErrEmptyWire) {
...
func TestWAL_IterateSequenced_OrderedAndFiltered(t *testing.T) {
	...
	for i := uint64(10); i < 15; i++ {
		...
		h := wireHashWal(wire)
		if err := c.Submit(ctx, h, wire, time.Now().UnixMicro(), nil); err != nil {
...
func TestWAL_GroupCommit_ConcurrentSubmits(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		go func(i int) {
			defer wg.Done()
			if err := c.Submit(ctx, hashes[i], wires[i], time.Now().UnixMicro(), nil); err != nil {
				errs <- err
			}
		}(i)
	}
