...
pipeline. Pairs with the v0.5.0 baseproof SDK release that ships
...
// SDKGhostLeafEmitterSnapshot is a point-in-time view of the running
// emit-disposition counters. Used by the chaos harness and SRE
// dashboards to characterise the crash-recovery rate. It is racy with
// concurrent Emit calls but each individual counter is atomic; the
// returned values are an internally-consistent point-in-time view.
