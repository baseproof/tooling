...
const cosignTestLogDID = "did:baseproof:integration:cosign-collection"

func requireIntegrationDSN(t *testing.T) *pgxpool.Pool {
	t.Helper()
	dsn := os.Getenv("BASEPROOF_TEST_DSN")
	if dsn == "" {
		t.Skip("BASEPROOF_TEST_DSN unset — use scripts/run-integration.sh")
	}
...
func TestCosign_QuorumCollectedAndPublished(t *testing.T) {
	...
	head := types.TreeHead{
		...
		SMTRoot:  [32]byte{0x42, 0x5A, 0x7C}, // non-zero: cosign Validate rejects all-zero SMTRoot
	}
...
func TestCosign_PublishOverwritesAtomically(t *testing.T) {
	...
	head1 := types.TreeHead{TreeSize: 100, RootHash: [32]byte{0xA1}, SMTRoot: [32]byte{0xB1}}
	...
	head2 := types.TreeHead{TreeSize: 200, RootHash: [32]byte{0xA2}, SMTRoot: [32]byte{0xB2}}
