...
DESCRIPTION:
	...
	(B3) under real PG outage. Skipped without BASEPROOF_TEST_DSN
	because driving the breaker's state machine requires either:

	  (a) BASEPROOF_TEST_DSN pointing at a Postgres the test can
...
func TestChaos_BreakerUnderRealPGOutage(t *testing.T) {
	if os.Getenv("BASEPROOF_TEST_DSN") == "" {
		t.Skip("BASEPROOF_TEST_DSN unset; breaker chaos requires a real PG with stop/start capability")
	}
	if os.Getenv("BASEPROOF_TEST_CHAOS_PG_OUTAGE") != "1" {
		t.Skip("BASEPROOF_TEST_CHAOS_PG_OUTAGE != 1; this test requires privileged PG control")
	}
